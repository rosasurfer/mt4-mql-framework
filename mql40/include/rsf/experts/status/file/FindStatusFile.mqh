/**
 * Find an existing status file for the passed instance id. Finds all files matching pattern:
 *
 *   "<program-name>,*<symbol>,*id=<instance-id>.set"
 *
 * located in "{sandbox-dir}/presets/{account-company}/". The search doesn't include subdirectories.
 * If multiple matching files are found only the last found (i.e. the most recent one) is returned.
 *
...
string FindStatusFile(int instanceId, bool isTest, int flags = NULL) {
   ...
   string filePattern = MqlProgramName() +",*"+ Symbol() +",*id="+ StrPadLeft(""+ instanceId, 3, "0") +".set";
