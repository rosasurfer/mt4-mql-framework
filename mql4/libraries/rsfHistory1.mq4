...
bool HistorySet1.AddTick(int hSet, datetime time, double value, int flags = NULL) {
   ...
   if (hSet != hs.hSet.lastValid) {
      ...
      if (hs.hSet[hSet] == 0)         return(!catch("HistorySet1.AddTick(3)  invalid parameter hSet: "+ hSet +" (unknown handle, symbol="+ DoubleQuoteStr(hs.symbol[hSet]) +")", ERR_INVALID_PARAMETER));
      if (hs.hSet[hSet] <  0)         return(!catch("HistorySet1.AddTick(4)  invalid parameter hSet: "+ hSet +" (closed handle, symbol="+ DoubleQuoteStr(hs.symbol[hSet]) +")", ERR_INVALID_PARAMETER));
      hs.hSet.lastValid = hSet;
   }
   if (time <= 0)                     return(!catch("HistorySet1.AddTick(5)  invalid parameter time: "+ time +" (symbol="+ DoubleQuoteStr(hs.symbol[hSet]) +")", ERR_INVALID_PARAMETER));
...
int HistoryFile1.Open(string symbol, int timeframe, string description, int digits, int format, int mode, string directory = "") {
   ...
   if (directory == "") {                                                        // current trade server: use MQL::FileOpenHistory()
      ...
      if (read_only) {
         ...
         if (hFile <= 0) return(!catch("HistoryFile1.Open(11)->FileOpen(\""+ filename +"\", FILE_READ) => "+ hFile +" ("+ symbol +","+ PeriodDescription(timeframe) +")", intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
      // read-write
      else if (read_write) {
         hFile = FileOpen(filename, mode|FILE_BIN);
         if (hFile <= 0) return(!catch("HistoryFile1.Open(12)->FileOpen(\""+ filename +"\", FILE_READ|FILE_WRITE) => "+ hFile +" ("+ symbol +","+ PeriodDescription(timeframe) +")", intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
      // write-only
      else if (write_only) {
         hFile = FileOpen(filename, mode|FILE_BIN);
         if (hFile <= 0) return(!catch("HistoryFile1.Open(13)->FileOpen(\""+ filename +"\", FILE_WRITE) => "+ hFile +" ("+ symbol +","+ PeriodDescription(timeframe) +")", intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
   }
   ...
   if (write_only || (read_write && fileSize < HISTORY_HEADER_size)) {
      ...
      if (digits < 0)                 return(!catch("HistoryFile1.Open(15)  invalid parameter digits: "+ digits +" ("+ symbol +","+ PeriodDescription(timeframe) +")", ERR_INVALID_PARAMETER));
      if (format!=400 && format!=401) return(!catch("HistoryFile1.Open(16)  invalid parameter format: "+ format +" (must be 400 or 401, symbol="+ symbol +","+ PeriodDescription(timeframe) +")", ERR_INVALID_PARAMETER));
      ...
      if (FileReadArray(hFile, hh, 0, HISTORY_HEADER_intSize) != HISTORY_HEADER_intSize) {
         FileClose(hFile);
         return(!catch("HistoryFile1.Open(17)  invalid history file \""+ filename +"\" (size="+ fileSize +")", intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
   ...
   return(!catch("HistoryFile1.Open(18)  "+ symbol +","+ PeriodDescription(timeframe), error));
}
