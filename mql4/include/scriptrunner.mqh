...
bool ScriptRunner.StopParamSender() {
   ...
   //debug("ScriptRunner.StopParamSender(2)  sender on "+ DoubleQuoteStr(ScriptRunner.GetChannelName()) +" stopped ("+ CoreFunctionToStr(__ExecutionContext[I_EC.coreFunction]) +")");
   return(true);
}
...
bool ScriptRunner.StopParamReceiver() {
   if (scriptrunner.hQC.receiver != NULL) {
      ...
      //debug("ScriptRunner.StopParamReceiver(2)  receiver on \""+ ScriptRunner.GetChannelName() +"\" stopped ("+ CoreFunctionToStr(__ExecutionContext[I_EC.coreFunction]) +")");
   }
   return(true);
}
