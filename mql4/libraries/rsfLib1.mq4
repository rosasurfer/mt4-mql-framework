 ...
 * functions determine whether an error is filtered and silently set or if an error causes a terminating ERROR alert. Filtered
 * errors must be handled by a custom error handler.
 *
 ...
 * @param  int    filter        - filter flags determining the errors to filter/pass-through
 * @param  int    oe[]          - order execution details (struct ORDER_EXECUTION)
 *
 * @return int - the same error
 */
int Order.HandleError(string message, int error, bool terminalError, int filter, int oe[]) {
   ...
   // always filter ERS_EXECUTION_STOPPING in Strategy Tester
...
bool OrderModifyEx(int ticket, double openPrice, double stopLoss, double takeProfit, datetime expires, color markerColor, int oeFlags, int oe[]) {
   ...
   if (!IsOrderType(OrderType()))                              return(_false(oe.setError(oe, Order.HandleError("OrderModifyEx(3)  #"+ ticket +" is not an order ticket", ERR_INVALID_TICKET, false, oeFlags, oe)), OrderPop("OrderModifyEx(4)")));
   if (OrderCloseTime() != 0)                                  return(_false(oe.setError(oe, Order.HandleError("OrderModifyEx(5)  #"+ ticket +" is already closed", ERR_INVALID_TICKET, false, oeFlags, oe)), OrderPop("OrderModifyEx(6)")));
   ...
   int error = GetLastError();
   if (IsError(error))                                         return(_false(oe.setError(oe, catch("OrderModifyEx(7)  symbol=\""+ OrderSymbol() +"\"", error, O_POP))));
   // openPrice
   openPrice = NormalizeDouble(openPrice, digits);
   if (LE(openPrice, 0, digits))                               return(_false(oe.setError(oe, catch("OrderModifyEx(8)  illegal parameter openPrice = "+ NumberToStr(openPrice, priceFormat), ERR_INVALID_PARAMETER, O_POP))));
   if (!EQ(openPrice, OrderOpenPrice(), digits)) {
      if (!isPendingType)                                      return(_false(oe.setError(oe, catch("OrderModifyEx(9)  cannot modify open price of already open position #"+ ticket, ERR_INVALID_PARAMETER, O_POP))));
   }
   // stopLoss
   stopLoss = NormalizeDouble(stopLoss, digits);
   if (LT(stopLoss, 0, digits))                                return(_false(oe.setError(oe, catch("OrderModifyEx(10)  illegal parameter stopLoss = "+ NumberToStr(stopLoss, priceFormat), ERR_INVALID_PARAMETER, O_POP))));
   // takeProfit
   takeProfit = NormalizeDouble(takeProfit, digits);
   if (LT(takeProfit, 0, digits))                              return(_false(oe.setError(oe, catch("OrderModifyEx(11)  illegal parameter takeProfit = "+ NumberToStr(takeProfit, priceFormat), ERR_INVALID_PARAMETER, O_POP))));
   // expires
   if (expires != 0)
      if (expires <= TimeCurrentEx("OrderModifyEx(12)"))       return(_false(oe.setError(oe, catch("OrderModifyEx(13)  illegal parameter expires = "+ ifString(expires < 0, expires, TimeToStr(expires, TIME_FULL)), ERR_INVALID_PARAMETER, O_POP))));
   if (expires != OrderExpiration())
      if (!isPendingType)                                      return(_false(oe.setError(oe, catch("OrderModifyEx(14)  cannot modify expiration of already open position #"+ ticket, ERR_INVALID_PARAMETER, O_POP))));
   // markerColor
   if (markerColor < CLR_NONE || markerColor > C'255,255,255') return(_false(oe.setError(oe, catch("OrderModifyEx(15)  illegal parameter markerColor = 0x"+ IntToHexStr(markerColor), ERR_INVALID_PARAMETER, O_POP))));
   ...
   if (EQ(openPrice, prevOpenPrice, digits)) /*&&*/ if (EQ(stopLoss, prevStopLoss, digits)) /*&&*/ if (EQ(takeProfit, prevTakeProfit, digits)) {
      warn("OrderModifyEx(16)  nothing to modify for #"+ ticket);
      return(!oe.setError(oe, catch("OrderModifyEx(17)", NULL, O_POP)));
   }
   ...
   while (true) {
      if (IsStopped()) return(_false(Order.HandleError("OrderModifyEx(18)  "+ OrderModifyEx.ErrorMsg(oe, prevOpenPrice, prevStopLoss, prevTakeProfit), ERS_EXECUTION_STOPPING, false, oeFlags, oe), OrderPop("OrderModifyEx(19)")));
      ...
      if (success) {
         ...
         if (!ChartMarker.OrderModified_A(ticket, digits, markerColor, TimeCurrentEx("OrderModifyEx(21)"), prevOpenPrice, prevStopLoss, prevTakeProfit))
            return(_false(oe.setError(oe, last_error), OrderPop("OrderModifyEx(22)")));

         // On slow OrderModify() or in a fast market limits may have already been executed or the order may have already been
         ...
         if (__LOG()) log("OrderModifyEx(23)  "+ OrderModifyEx.SuccessMsg(oe, prevOpenPrice, prevStopLoss, prevTakeProfit));
         if (!IsTesting()) PlaySoundEx("OrderModified.wav");
         return(!oe.setError(oe, catch("OrderModifyEx(24)", NULL, O_POP)));            // regular exit (NO_ERROR)
      }

      error = oe.setError(oe, GetLastError());
      if (error == ERR_TRADE_CONTEXT_BUSY) {
         if (__LOG()) log("OrderModifyEx(25)  trade context busy, retrying...");
         Sleep(300);
         continue;
      }
      ...
      switch (error) {                                                                 // continue on temporary errors
         ...
         case ERR_TRADE_TIMEOUT:
            tempErrors++; if (tempErrors <= 5) {
               warn("OrderModifyEx(26)  "+ Order.TempErrorMsg(oe, tempErrors), error);
   ...
   return(_false(Order.HandleError("OrderModifyEx(27)  "+ msg, error, true, oeFlags, oe), OrderPop("OrderModifyEx(28)")));
}
 ...
 *
 * TODO: Handle ERR_INVALID_TRADE_PARAMETERS (a ticket may be closed by two parallel requests).
 */
bool OrderCloseEx(int ticket, double lots, double price, double slippage, color markerColor, int oeFlags, /*ORDER_EXECUTION*/int oe[]) {
   ...
   */
   ...
   while (true) {
      if (IsStopped()) return(_false(Order.HandleError("OrderCloseEx(12)  "+ OrderCloseEx.ErrorMsg(oe), ERS_EXECUTION_STOPPING, false, oeFlags, oe), OrderPop("OrderCloseEx(13)")));
 ...
 *            - ERR_INVALID_TICKET:           one of the ids is not a known ticket id
 *            - ERR_INVALID_TRADE_PARAMETERS: one of the tickets is not an open position or the tickets can't be closed by
 *                                            each other (anymore)
 *
 *        (2) On partial close the resulting values for swap, commission and profit may be divided over both tickets by the
 *            trade server. The returned values are summed-up values of both tickets.
 */
bool OrderCloseByEx(int ticket, int opposite, color markerColor, int oeFlags, int oe[]) {
   ...
   if (OrderCloseTime() != 0)                                     return(_false(oe.setError(oe, catch("OrderCloseByEx(2)  #"+ ticket +" is already closed", ERR_INVALID_TRADE_PARAMETERS, O_POP))));
   if (OrderType() > OP_SELL)                                     return(_false(oe.setError(oe, catch("OrderCloseByEx(3)  #"+ ticket +" is not an open position", ERR_INVALID_TRADE_PARAMETERS, O_POP))));
   ...
   if (OrderCloseTime() != 0)                                     return(_false(oe.setError(oe, catch("OrderCloseByEx(5)  opposite #"+ opposite +" is already closed", ERR_INVALID_TRADE_PARAMETERS, O_POP))));
   int      oppositeType     = OrderType();
   double   oppositeLots     = OrderLots();
   datetime oppositeOpenTime = OrderOpenTime();
   if (ticketType != oppositeType^1)                              return(_false(oe.setError(oe, catch("OrderCloseByEx(6)  #"+ opposite +" is not opposite to #"+ ticket, ERR_INVALID_TRADE_PARAMETERS, O_POP))));
   if (symbol != OrderSymbol())                                   return(_false(oe.setError(oe, catch("OrderCloseByEx(7)  #"+ opposite +" is not opposite to #"+ ticket, ERR_INVALID_TRADE_PARAMETERS, O_POP))));
   // markerColor
   ...
   */
   ...
   while (true) {
      if (IsStopped()) return(_false(Order.HandleError(StringConcatenate("OrderCloseByEx(9)  ", OrderCloseByEx.ErrorMsg(first, second, oe)), ERS_EXECUTION_STOPPING, false, oeFlags, oe), OrderPop("OrderCloseByEx(10)")));
      ...
      if (success) {
         ...
         if (!EQ(firstLots, secondLots)) {
            ...
            if (smallerByLarger) {                                               // online
               ...
               if (!remainder) return(_false(oe.setError(oe, catch("OrderCloseByEx(12)  cannot find remaining position of close #"+ ticket +" ("+ NumberToStr(ticketLots, ".+") +" lots = smaller) by #"+ opposite +" ("+ NumberToStr(oppositeLots, ".+") +" lots = larger)", ERR_RUNTIME_ERROR, O_POP))));
            }

            else /*(largerBySmaller)*/ {                                         // in Strategy Tester
               // no reference available
               if (!SelectTicket(larger, "OrderCloseByEx(13)", NULL, O_POP)) return(_false(oe.setError(oe, last_error)));
         ...
         return(!oe.setError(oe, catch("OrderCloseByEx(17)", NULL, O_POP)));     // regular exit (NO_ERROR)
      }
      ...
      switch (error) {
         ...
         case ERR_TRADE_TIMEOUT:
            ...
            if (tempErrors > 5)
               break;
            warn("OrderCloseByEx(19)  "+ Order.TempErrorMsg(oe, tempErrors), error);
            continue;
         ...
         case ERR_INVALID_TICKET:                 // unknown tickets or not open positions anymore (client-side)                          ! not yet encountered
            if (__LOG()) log("OrderCloseByEx(20)  translating returned ERR_INVALID_TICKET => ERR_INVALID_TRADE_PARAMETERS");
         ...
         case ERR_OFF_QUOTES:                     // positions are locked and in processing queue (server-side) => SL/TP are executed     ! not yet encountered
            if (__LOG()) log("OrderCloseByEx(21)  translating returned ERR_OFF_QUOTES => ERR_INVALID_TRADE_PARAMETERS");
            error = ERR_INVALID_TRADE_PARAMETERS;
            break;
         case ERR_INVALID_TRADE_PARAMETERS:       // positions are processed and not open anymore (server-side)
            break;
         case NO_ERROR:
            if (__LOG()) log("OrderCloseByEx(22)  no error returned => ERR_RUNTIME_ERROR");
            error = ERR_RUNTIME_ERROR;
            break;
      }
      break;                                      // fail for all other errors
   }
   return(_false(oe.setError(oe, Order.HandleError("OrderCloseByEx(23)  "+ OrderCloseByEx.ErrorMsg(first, second, oe), error, true, oeFlags, oe)), OrderPop("OrderCloseByEx(24)")));
}
...
bool OrderCloseMulti(int tickets[], double slippage, color markerColor, int oeFlags, /*ORDER_EXECUTION*/int oes[][]) {
   // validate parameters
   // oes[]
   if (ArrayDimension(oes) != 2)                               return(!catch("OrderCloseMulti(1)  invalid parameter oes[] (illegal number of dimensions: "+ ArrayDimension(oes) +")", ERR_INCOMPATIBLE_ARRAYS));
   if (ArrayRange(oes, 1) != ORDER_EXECUTION.intSize)          return(!catch("OrderCloseMulti(2)  invalid size of parameter oes["+ ArrayRange(oes, 0) +"]["+ ArrayRange(oes, 1) +"]", ERR_INCOMPATIBLE_ARRAYS));
   int sizeOfTickets = ArraySize(tickets);
   ArrayResize(oes, sizeOfTickets); ArrayInitialize(oes, 0);

   // tickets
   if (sizeOfTickets == 0)                                     return(!oes.setError(oes, -1, catch("OrderCloseMulti(3)  invalid size "+ sizeOfTickets +" of parameter tickets = {}", ERR_INVALID_PARAMETER)));
   OrderPush("OrderCloseMulti(4)");
   for (int i=0; i < sizeOfTickets; i++) {
      if (!SelectTicket(tickets[i], "OrderCloseMulti(5)", NULL, O_POP))
         return(_false(oes.setError(oes, -1, last_error)));
      if (OrderCloseTime() != 0)                               return(!oes.setError(oes, -1, catch("OrderCloseMulti(6)  #"+ tickets[i] +" is already closed", ERR_INVALID_TICKET, O_POP)));
      if (OrderType() > OP_SELL)                               return(!oes.setError(oes, -1, catch("OrderCloseMulti(7)  #"+ tickets[i] +" is not an open position", ERR_INVALID_TICKET, O_POP)));
   }
   // slippage
   if (LT(slippage, 0))                                        return(!oes.setError(oes, -1, catch("OrderCloseMulti(8)  illegal parameter slippage = "+ NumberToStr(slippage, ".+"), ERR_INVALID_PARAMETER, O_POP)));
   // markerColor
   if (markerColor < CLR_NONE || markerColor > C'255,255,255') return(!oes.setError(oes, -1, catch("OrderCloseMulti(9)  illegal parameter markerColor = 0x"+ IntToHexStr(markerColor), ERR_INVALID_PARAMETER, O_POP)));
   ...
   if (sizeOfTickets == 1) {
      ...
      if (!OrderCloseEx(tickets[0], NULL, NULL, slippage, markerColor, oeFlags, oe))
         return(_false(oes.setError(oes, -1, last_error), OrderPop("OrderCloseMulti(10)")));
      CopyMemory(GetIntsAddress(oes), GetIntsAddress(oe), ArraySize(oe)*4);
      ArrayResize(oe, 0);
      return(OrderPop("OrderCloseMulti(11)") && !oes.setError(oes, -1, last_error));
   }
   ...
   for (i=0; i < sizeOfTickets; i++) {
      if (!SelectTicket(tickets[i], "OrderCloseMulti(12)", NULL, O_POP))
         return(_false(oes.setError(oes, -1, last_error)));
   ...
   if (sizeOfSymbols == 1) {
      if (!OrderCloseMulti.OneSymbol(tickets, slippage, markerColor, oeFlags, oes2))
         return(_false(oes.setError(oes, -1, last_error), OrderPop("OrderCloseMulti(13)")));
      ...
      return(OrderPop("OrderCloseMulti(14)") && !oes.setError(oes, -1, last_error));
   }

   // (5) Tickets gehören zu mehreren Symbolen
   if (__LOG()) log(StringConcatenate("OrderCloseMulti(15)  closing ", sizeOfTickets, " mixed positions ", TicketsToStr.Lots(tickets, NULL)));

   // (5.1) oes[] vorbelegen
   for (i=0; i < sizeOfTickets; i++) {
      if (!SelectTicket(tickets[i], "OrderCloseMulti(16)", NULL, O_POP))
         return(_false(oes.setError(oes, -1, last_error)));
   ...
   if (!OrderPop("OrderCloseMulti(17)"))
      return(_false(oes.setError(oes, -1, last_error)));
   ...
   for (si=0; si < sizeOfSymbols; si++) {
      ...
      int newTicket = OrderCloseMulti.Hedge(group, slippage, oeFlags, oes2);   // -1: kein neues Ticket
   ...
   return(!oes.setError(oes, -1, catch("OrderCloseMulti(18)")));
}
 ...
 */
int OrderCloseMulti.Hedge(int tickets[], double slippage, int oeFlags, /*ORDER_EXECUTION*/int oes[][]) {
   // keine nochmalige, ausführliche Parametervalidierung (da private)
   int sizeOfTickets = ArraySize(tickets);
   if (sizeOfTickets == 0)
      return(_NULL(oes.setError(oes, -1, catch("OrderCloseMulti.Hedge(1)  invalid parameter tickets, size = "+ sizeOfTickets, ERR_INVALID_PARAMETER))));


   // (1) oes[] vorbelegen, dabei Lotsizes und Gesamtposition ermitteln
   ArrayResize(oes, sizeOfTickets); ArrayInitialize(oes, 0);
   if (!SelectTicket(tickets[0], "OrderCloseMulti.Hedge(2)", O_PUSH))
   ...
   for (int i=0; i < sizeOfTickets; i++) {
      if (!SelectTicket(tickets[i], "OrderCloseMulti.Hedge(3)", NULL, O_POP))
         return(_NULL(oes.setError(oes, -1, last_error)));
   ...
   if (EQ(totalLots, 0)) {
      if (__LOG()) log(StringConcatenate("OrderCloseMulti.Hedge(4)  ", sizeOfTickets, " ", symbol, " positions ", TicketsToStr.Lots(tickets, NULL), " are already flat"));
      ...
      if (!SelectTicket(tickets.copy[sizeOfTickets-1], "OrderCloseMulti.Hedge(5)", NULL, O_POP))
         return(_NULL(oes.setError(oes, -1, last_error)));
      ...
      if (!OrderPop("OrderCloseMulti.Hedge(6)"))
         return(_NULL(oes.setError(oes, -1, last_error)));
      ArrayResize(tickets.copy, 0);
   }
   else {
      if (!OrderPop("OrderCloseMulti.Hedge(7)"))
         return(_NULL(oes.setError(oes, -1, last_error)));
      if (__LOG()) log(StringConcatenate("OrderCloseMulti.Hedge(8)  flattening ", sizeOfTickets, " ", symbol, " position", ifString(sizeOfTickets==1, " ", "s "), TicketsToStr.Lots(tickets, NULL)));
   ...
   if (!catch("OrderCloseMulti.Hedge(9)"))
      return(newTicket);
   return(_NULL(oes.setError(oes, -1, last_error)));
}
 ...
 *
 * @param  _In_  int   tickets[]   - Tickets der ausgeglichenen Positionen
 * @param  _In_  color markerColor - Farbe des Chart-Markers
 * @param  _In_  int   oeFlags     - die Ausführung steuernde Flags
 * @param  _Out_ int   oes[]       - Ausführungsdetails (ORDER_EXECUTION[])
 *
 * @return bool - success status
 *
 *
 ...
 */
bool OrderCloseHedged(int tickets[], color markerColor, int oeFlags, /*ORDER_EXECUTION*/int oes[][]) {
   // keine nochmalige ausführliche Parametervalidierung, da private
   int sizeOfTickets = ArraySize(tickets);
   if (sizeOfTickets < 2)
      return(_false(oes.setError(oes, -1, catch("OrderCloseHedged(1)  invalid parameter tickets, size = "+ sizeOfTickets, ERR_INVALID_PARAMETER))));
   ArrayResize(oes, sizeOfTickets); ArrayInitialize(oes, 0);

   // oes[] vorbelegen
   if (!SelectTicket(tickets[0], "OrderCloseHedged(2)", O_PUSH))
      return(_false(oes.setError(oes, -1, last_error)));
   int digits = MarketInfo(OrderSymbol(), MODE_DIGITS);

   for (int i=0; i < sizeOfTickets; i++) {
      if (!SelectTicket(tickets[i], "OrderCloseHedged(3)", NULL, O_POP))
         return(_false(oes.setError(oes, -1, last_error)));
      oes.setSymbol    (oes, i, OrderSymbol()    );
      oes.setDigits    (oes, i, digits           );
   ...
   if (__LOG()) log(StringConcatenate("OrderCloseHedged(4)  closing ", sizeOfTickets, " hedged ", OrderSymbol(), " positions ", TicketsToStr.Lots(tickets, NULL)));

   // tickets[] wird in Folge modifiziert. Um Änderungen am übergebenen Array zu vermeiden, arbeiten wir auf einer Kopie.
   int tickets.copy[]; ArrayResize(tickets.copy, 0);
   int sizeOfCopy = ArrayCopy(tickets.copy, tickets);

   SortTicketsChronological(tickets.copy);
   if (!SelectTicket(tickets.copy[sizeOfCopy-1], "OrderCloseHedged(5)", NULL, O_POP))  // das zuletzt geöffnete Ticket
      return(_false(oes.setError(oes, -1, last_error)));
   for (i=0; i < sizeOfTickets; i++) {
      oes.setCloseTime (oes, i, OrderOpenTime() );
      oes.setClosePrice(oes, i, OrderOpenPrice());
   ...
   while (sizeOfCopy > 0) {
      ...
      if (!SelectTicket(first, "OrderCloseHedged(6)", NULL, O_POP))
         return(_false(oes.setError(oes, -1, last_error)));
      int firstType = OrderType();

      for (i=1; i < sizeOfCopy; i++) {
         if (!SelectTicket(tickets.copy[i], "OrderCloseHedged(7)", NULL, O_POP))
            return(_false(oes.setError(oes, -1, last_error)));
      ...
      if (!opposite)
         return(_false(oes.setError(oes, -1, catch("OrderCloseHedged(8)  cannot find opposite position for "+ OperationTypeDescription(firstType) +" #"+ first, ERR_RUNTIME_ERROR, O_POP))));

      int oe[];
      if (!OrderCloseByEx(first, opposite, markerColor, oeFlags, oe))                  // erste und Opposite-Position schließen
         return(_false(oes.setError(oes, -1, last_error), OrderPop("OrderCloseHedged(9)")));
   ...
   ArrayResize(oe,           0);
   ArrayResize(tickets.copy, 0);
   return(!oes.setError(oes, -1, catch("OrderCloseHedged(10)", NULL, O_POP)));
}
 ...
 *          die Summe der Einzelwerte aller Tickets entspricht jedoch dem tatsächlichen Gesamtwert.
 */
bool OrderCloseMulti.OneSymbol(int tickets[], double slippage, color markerColor, int oeFlags, /*ORDER_EXECUTION*/int oes[][]) {
   // keine nochmalige, ausführliche Parametervalidierung (da private)
   int sizeOfTickets = ArraySize(tickets);
   if (sizeOfTickets == 0)
      return(!oes.setError(oes, -1, catch("OrderCloseMulti.OneSymbol(1)  invalid parameter tickets, size = "+ sizeOfTickets, ERR_INVALID_PARAMETER)));
   ArrayResize(oes, sizeOfTickets); ArrayInitialize(oes, 0);
   ...
   if (__LOG()) log(StringConcatenate("OrderCloseMulti.OneSymbol(2)  closing ", sizeOfTickets, " ", OrderSymbol(), " positions ", TicketsToStr.Lots(tickets, NULL)));


   // (2) oes[] vorbelegen
   if (!SelectTicket(tickets[0], "OrderCloseMulti.OneSymbol(3)", O_PUSH))
      return(_false(oes.setError(oes, -1, last_error)));
   int digits = MarketInfo(OrderSymbol(), MODE_DIGITS);

   for (i=0; i < sizeOfTickets; i++) {
      if (!SelectTicket(tickets[i], "OrderCloseMulti.OneSymbol(4)", NULL, O_POP))
         return(_false(oes.setError(oes, -1, last_error)));
      oes.setSymbol    (oes, i, OrderSymbol()    );
      oes.setDigits    (oes, i, digits           );
      oes.setTicket    (oes, i, tickets[i]       );
   ...
   int newTicket = OrderCloseMulti.Hedge(tickets.copy, slippage, oeFlags, oes2);                 // -1: kein neues Ticket
   if (IsLastError())                                                                              //  0: Fehler oder Gesamtposition war bereits flat
      return(_false(oes.setError(oes, -1, last_error), OrderPop("OrderCloseMulti.OneSymbol(5)"))); // >0: neues Ticket
   ...
   if (!OrderCloseHedged(tickets.copy, markerColor, oeFlags, oes2))
      return(_false(oes.setError(oes, -1, last_error), OrderPop("OrderCloseMulti.OneSymbol(6)")));
   ...
   ArrayResize(oes2,         0);
   ArrayResize(tickets.copy, 0);
   return(!oes.setError(oes, -1, catch("OrderCloseMulti.OneSymbol(7)", NULL, O_POP)));
}
...
bool OrderDeleteEx(int ticket, color markerColor, int oeFlags, int oe[]) {
   ...
   if (!IsPendingOrderType(OrderType()))                       return(_false(oe.setError(oe, Order.HandleError("OrderDeleteEx(3)  #"+ ticket +" is not a pending order", ERR_INVALID_TRADE_PARAMETERS, false, oeFlags, oe)), OrderPop("OrderDeleteEx(4)")));
   if (OrderCloseTime() != 0)                                  return(_false(oe.setError(oe, Order.HandleError("OrderDeleteEx(5)  #"+ ticket +" is already deleted", ERR_INVALID_TRADE_PARAMETERS, false, oeFlags, oe)), OrderPop("OrderDeleteEx(6)")));
   // markerColor
   if (markerColor < CLR_NONE || markerColor > C'255,255,255') return(_false(oe.setError(oe, Order.HandleError("OrderDeleteEx(7)  illegal parameter markerColor = 0x"+ IntToHexStr(markerColor), ERR_INVALID_PARAMETER, false, oeFlags, oe)), OrderPop("OrderDeleteEx(8)")));
   ...
   */
   ...
   while (true) {
      if (IsStopped()) return(_false(oe.setError(oe, Order.HandleError("OrderDeleteEx(9)  "+ OrderDeleteEx.ErrorMsg(oe), ERS_EXECUTION_STOPPING, false, oeFlags, oe)), OrderPop("OrderDeleteEx(10)")));
