...
bool StartSequence(int signal) {
   ...
   if (IsLog()) log("StartSequence(2)  "+ sequence.name +" starting sequence at level "+ sequence.level +"..."+ ifString(gridbase, " (predefined gridbase "+ NumberToStr(gridbase, PriceFormat) +")", ""));
   ...
   if (IsLog()) log("StartSequence(6)  "+ sequence.name +" sequence started at level "+ sequence.level +" (start price "+ NumberToStr(startPrice, PriceFormat) +", gridbase "+ NumberToStr(gridbase, PriceFormat) +")");
...
bool StopSequence(int signal) {
   ...
   if (sequence.status == STATUS_WAITING) {
      ...
      if (IsLog()) log("StopSequence(2)  "+ sequence.longName +" stopped");
   }
   ...
   // a progressing sequence has open orders to close
      ...
      if (IsLog()) log("StopSequence(3)  "+ sequence.longName +" stopping sequence...");
      ...
      for (i=0; i < sizeOfPendings; i++) {                                 // ordered by descending gridlevel
         if (orders.type[pendingLimits[i]] == OP_UNDEFINED) {
            ...
            if (IsLog()) log("StopSequence(5)  "+ sequence.longName +" "+ UpdateStatus.OrderFillMsg(pendingLimits[i]));
            ...
            if (IsLog()) log("StopSequence(6)  "+ sequence.longName +" adding ticket #"+ OrderTicket() +" to open positions");
            ...
            orders.commission[pendingLimits[i]] = OrderCommission();
            orders.profit    [pendingLimits[i]] = OrderProfit();
            if (IsLog()) log("StopSequence(8)  "+ sequence.longName +" "+ UpdateStatus.StopLossMsg(pendingLimits[i]));
            sequence.stops++;
      ...
      sequence.status = STATUS_STOPPED;
      if (IsLog()) log("StopSequence(10)  "+ sequence.name +" sequence stopped at level "+ sequence.level +" (stop price "+ NumberToStr(stopPrice, PriceFormat) +", gridbase "+ NumberToStr(GetGridbase(), PriceFormat) +")");
      UpdateProfitTargets();
      ShowProfitTargets();
      SS.ProfitPerLevel();
   }
...
bool ResetSequence(double gridbase, int level) {
   ...
   if (IsLog()) log("ResetSequence(4)  "+ sequence.name +" sequence reset to level "+ sequence.level +" ("+ ifString(gridbase, "new gridbase "+ NumberToStr(gridbase, PriceFormat) +", ", "") +"status "+ DoubleQuoteStr(StatusDescription(sequence.status)) +")");
...
bool ResumeSequence(int signal) {
   ...
   sequence.status = STATUS_STARTING;
   if (IsLog()) log("ResumeSequence(2)  "+ sequence.name +" resuming sequence at level "+ sequence.level +" (stop price "+ NumberToStr(stopPrice, PriceFormat) +", old gridbase "+ NumberToStr(lastGridbase, PriceFormat) +")");
   ...
   if (IsLog()) log("ResumeSequence(5)  "+ sequence.name +" resumed at level "+ sequence.level +" (start price "+ NumberToStr(startPrice, PriceFormat) +", new gridbase "+ NumberToStr(newGridbase, PriceFormat) +")");
...
bool RestorePositions(datetime &lpOpenTime, double &lpOpenPrice) {
   ...
   if (level != 0) {
      if (orders.ticket[i] == -1) {
         if (IsLog()) log("RestorePositions(2)  "+ sequence.longName +" "+ UpdateStatus.StopLossMsg(i));
   ...
   for (int level, i=sizeOfTickets-1; i >= 0; i--) {
      ...
      if (wasPending) {
         ...
         if (OrderType() != orders.pendingType[i]) {                          // a pending entry order was executed
            ...
            if (IsLog()) log("UpdateStatus(3)  "+ sequence.longName +" "+ UpdateStatus.OrderFillMsg(i));
      ...
      if (!isClosed) {                                                        // a still open order
         ...
         if (orders.closedBySL[i]) {                                          // stopped out
            if (IsLog()) {             log("UpdateStatus(6)  "+ sequence.longName +" "+ UpdateStatus.StopLossMsg(i));
            ...
            if (IsLog()) log("UpdateStatus(9)  "+ sequence.longName +" "+ UpdateStatus.PositionCloseMsg(i));
...
bool IsStartSignal(int &signal) {
      ...
      if (start.conditions && start.trend.condition) {
         if (IsBarOpenEvent(start.trend.timeframe)) {
            ...
            if ((sequence.direction==D_LONG && trend==1) || (sequence.direction==D_SHORT && trend==-1)) {
               if (IsLog()) log("IsStartSignal(1)  "+ sequence.longName +" queuing fulfilled "+ ifString(!resuming, "start", "resume") +" condition \"@"+ start.trend.description +"\"");
   ...
   if (sessionbreak.waiting) {
      ...
      if (!sequence.level) {
         if (IsLog()) log("IsStartSignal(2)  "+ sequence.longName +" resume condition \"@sessionbreak in level 0\" fulfilled ("+ ifString(sequence.direction==D_LONG, "ask", "bid") +": "+ NumberToStr(ifDouble(sequence.direction==D_LONG, Ask, Bid), PriceFormat) +")");
      ...
      if (triggered) {
         if (IsLog()) log("IsStartSignal(3)  "+ sequence.longName +" resume condition \"@sessionbreak price "+ NumberToStr(price, PriceFormat) +"\" fulfilled ("+ ifString(sequence.direction==D_LONG, "ask", "bid") +": "+ NumberToStr(ifDouble(sequence.direction==D_LONG, Ask, Bid), PriceFormat) +")");
         signal = SIGNAL_SESSIONBREAK;
         return(true);
      }
...
bool IsSessionBreak() {
   ...
   if (serverTime >= sessionbreak.endtime) {
      ...
      if (IsLog()) log("IsSessionBreak(1)  "+ sequence.longName +" recalculated next sessionbreak: from "+ GmtTimeFormat(sessionbreak.starttime, "%a, %Y.%m.%d %H:%M:%S") +" to "+ GmtTimeFormat(sessionbreak.endtime, "%a, %Y.%m.%d %H:%M:%S"));
   }
   ...
   */
   ...
   for (int i=sizeOfTickets-1; i >= 0; i--) {
      if (orders.level[i] == level) {
         ...
         if (!orders.closeTime[i]) {                                       // order is open, check the SL of level 1 orders
            ...
            if (IsLog()) log("UpdatePendingOrders(5)  "+ sequence.longName +" re-opening closed level "+ level +" order...");
            ...
            if (IsLimitOrderType(type)) {                                  // add limit order to missed levels
               ...
               if (IsLog()) log("UpdatePendingOrders(6)  "+ sequence.longName +" re-opened order is a stop order, decreasing sequence level...");
               nextLevel       = level;
   ...
   if (level != 0) {
      ...
      while (true) {                                                       // with a level but no orders we are in StartSequence() with a predefined sequence.level != 0
         ...
         if (IsLimitOrderType(type)) {                                     // add limit order to missed levels
            ...
            if (IsLog()) log("UpdatePendingOrders(8)  "+ sequence.longName +" opened order is a stop order, decreasing sequence level...");
            sequence.level = level - levelStep; SS.SequenceName();
            nextLevel      = level;
   ...
   while (idxNextStop == -1) {                                             // open a missing next stop order
      ...
      if (IsStopOrderType(type)) {                                         // a stop order was opened
         ...
         if (IsLog()) log("UpdatePendingOrders(11)  "+ sequence.longName +" submitted order is a limit order, increasing sequence level...");
         idxCurrentLevel   = sizeOfTickets-1;
   ...
   if (newLimitOrders > 0) {
      ...
      if (IsLog()) log("UpdatePendingOrders(12)  "+ sequence.longName +" opened "+ newLimitOrders +" limit order"+ Pluralize(newLimitOrders) +" for missed level"+ Pluralize(newLimitOrders) +" "+ sMissedLevels +" (all missed levels: "+ JoinInts(sequence.missedLevels) +")");
   }
   UpdateProfitTargets();
   ShowProfitTargets();
   SS.ProfitPerLevel();
...
bool UpdatePendingOrders.DeleteError(int i, int error, int saveStatusMode) {
   if (error == -1) {                                                   // the order was already executed
      if (IsLog()) log("UpdatePendingOrders.DeleteError(1)  "+ sequence.longName +" pending stop order for level "+ orders.level[i] +" was already executed (#"+ orders.ticket[i] +")");
      bool bNull;
...
int Grid.AddPendingOrder(int level, int offset=-1) {
   ...
   while (true) {
      ...
      if (ticket == -1) {                          // the same pending order type again and again caused by a stalled price feed
         if (IsLog()) log("Grid.AddPendingOrder(3)  "+ sequence.longName +" illegal price "+ OperationTypeDescription(pendingType) +" at "+ NumberToStr(oe.OpenPrice(oe), PriceFormat) +" (market "+ NumberToStr(oe.Bid(oe), PriceFormat) +"/"+ NumberToStr(oe.Ask(oe), PriceFormat) +"), opening "+ ifString(IsStopOrderType(pendingType), "limit", "stop") +" order instead", error);
...
bool Grid.AddPosition(int level) {
   ...
   if (ticket <= 0) {
      ...
      if (ticket == -1) {
         ...
         if (IsLog()) log("Grid.AddPosition(4)  "+ sequence.longName +" new position at level "+ level +" would be immediately closed by SL="+ NumberToStr(oe.StopLoss(oe), PriceFormat) +", adding marker ticket #-1 (market: "+ NumberToStr(oe.Bid(oe), PriceFormat) +"/"+ NumberToStr(oe.Ask(oe), PriceFormat) +")");
      }
      else if (ticket == -2) {
   ...
   if (IsError(error)) {
      ...
      if (error == -1) {                                    // market violated: delete stop order and open a limit order instead
         ...
         if (error == -1) {                                 // deletion failed, the stop order was already executed
            if (IsLog()) log("Grid.TrailPendingOrder(6)  "+ sequence.longName +" pending #"+ orders.ticket[i] +" was already executed");
...
int SetLastNetworkError(int oe[]) {
   ...
   if (!error) {
      ...
      if (IsLog()) log("SetLastNetworkError(2)  "+ sequence.longName +" networkError "+ ErrorToStr(lastNetworkError) +", next trade request not before "+ TimeToStr(nextRetry, TIME_FULL));
   }
   return(error);
}
