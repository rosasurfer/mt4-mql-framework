...
bool ManageIntIndicatorBuffer(int id, int buffer[], int emptyValue = 0) {
   ...
   if (__CoreFunction != CF_START) return(!catch("ManageIntIndicatorBuffer(2)  id="+ id +", invalid calling context: "+ ProgramTypeDescription(__ExecutionContext[EC.programType]) +"::"+ CoreFunctionDescription(__CoreFunction), ERR_ILLEGAL_STATE));
   if (!Bars)                      return(!catch("ManageIntIndicatorBuffer(3)  id="+ id +", Tick="+ Ticks +"  Bars=0", ERR_ILLEGAL_STATE));
   ...
   if (Bars == prevBars) {
      ...
      if (!ValidBars) {
         ...
         if (!ShiftedBars || Time[ShiftedBars]!=prevNewestBarTime) {
            return(!catch("ManageIntIndicatorBuffer(4)  id="+ id +", Tick="+ Ticks +", Bars unchanged but oldest bar changed, hit the timeseries MAX_CHART_BARS? (Bars="+ Bars +", ShiftedBars="+ ShiftedBars +", oldestBarTime="+ TimeToStr(Time[Bars-1], TIME_FULL) +", prevOldestBarTime="+ TimeToStr(prevOldestBarTime, TIME_FULL) +")", ERR_ILLEGAL_STATE));
         }
      }
   }
   else {
      if (!ValidBars) {
         ...
         if (Bars > prevBars) {                                      // the number of Bars increased (new bars have been inserted or appended anywhere)
            if (prevBars && Time[Bars-1]!=prevOldestBarTime) {       // the oldest bar changed: bars have been added at the end (data pumping)
               return(!catch("ManageIntIndicatorBuffer(5)  id="+ id +", Tick="+ Ticks +", Bars increased and oldest bar changed but no full recalculation (Bars="+ Bars +", prevBars="+ prevBars +", oldestBarTime="+ TimeToStr(Time[Bars-1], TIME_FULL) +", prevOldestBarTime="+ TimeToStr(prevOldestBarTime, TIME_FULL) +", ValidBars="+ ValidBars +")", ERR_ILLEGAL_STATE));
            }
         }
            ...
            if (i == Bars) return(!catch("ManageIntIndicatorBuffer(6)  id="+ id +", Tick="+ Ticks +", Bars decreased from "+ prevBars +" to "+ Bars +" but previous Time[0] not found", ERR_ILLEGAL_STATE));
            ...
            if (IsLogInfo()) logInfo("ManageIntIndicatorBuffer(6.1)  id="+ id +", Tick="+ Ticks +", Bars decreased from "+ prevBars +" to "+ Bars +" (previous Time[0] bar found at offset "+ i +")");
         }
      }
