...
/*sticky*/ int    startStopDisplayMode = SDM_PRICE;                  // "sticky" Runtime-Variablen werden im Chart zwischengespeichert,
/*sticky*/ int    orderDisplayMode     = ODM_NONE;                   // sie überleben dort Terminal-Restart, Profilwechsel und Recompilation
...
bool Strategy(int hSeq) {
   ...
   // (3) ... oder läuft.
      ...
      if (IsStopSignal(hSeq, takeProfit, weekend)) StopSequence(hSeq, takeProfit, weekend);
         ...
         if (ArraySize(stops) > 0)                 ProcessLocalLimits(hSeq, stops);
...
bool IsWeekendResumeSignal(int hSeq) {
   ...
   if (sequence.weResumeTime[hSeq] <= now) {
      if (__LOG()) log(StringConcatenate("IsWeekendResumeSignal(3)  resume condition '", GmtTimeFormat(sequence.weResumeTime[hSeq], "%a, %Y.%m.%d %H:%M:%S"), "' met"));
      return(true);
   }
   return(false);
}
...
bool IsStopSignal(int hSeq, bool &lpTakeProfitStop, bool &lpWeekendStop) {
   if (IsLastError() || sequence.status[hSeq]!=STATUS_PROGRESSING)
      return(false);
...
/**
 * Ordermanagement getriggerter client-seitiger Limits. Kann eine getriggerte Stop-Order oder ein getriggerter Stop-Loss sein.
...
bool ProcessLocalLimits(int hSeq, int stops[]) {
   ...
   for (int i, n=0; n < sizeOfStops; n++) {
      i = stops[n];
      if (i < ticketsFrom || i > ticketsTo)  return(!catch("ProcessLocalLimits(3)  illegal value "+ i +" in parameter stops = "+ IntsToStr(stops, NULL), ERR_INVALID_PARAMETER));


      // (2) getriggerte Pending-Order (OP_BUYSTOP, OP_SELLSTOP)
      if (orders.ticket[i] == -1) {
         if (orders.type[i] != OP_UNDEFINED) return(!catch("ProcessLocalLimits(4)  client-side "+ OperationTypeDescription(orders.pendingType[i]) +" order at index "+ i +" already marked as open", ERR_ILLEGAL_STATE));
         ...
         if (ticket <= 0) {
            if (level != sequence.level[hSeq])    return(false);
            if (oe.Error(oe) != ERR_INVALID_STOP) return(false);
            if (ticket==0 || ticket < -2)         return(!catch("ProcessLocalLimits(5)", oe.Error(oe)));

            double stopLoss = oe.StopLoss(oe);

            // (2.2) Spread violated
            if (ticket == -1) {
               return(!catch("ProcessLocalLimits(6)  spread violated ("+ NumberToStr(oe.Bid(oe), PriceFormat) +"/"+ NumberToStr(oe.Ask(oe), PriceFormat) +") by "+ OperationTypeDescription(type) +" at "+ NumberToStr(oe.OpenPrice(oe), PriceFormat) +", sl="+ NumberToStr(stopLoss, PriceFormat) +" (level "+ level +")", oe.Error(oe)));
            }
      ...
      if (orders.clientSL[i]) {
         if (orders.ticket[i] == -2)         return(!catch("ProcessLocalLimits(8)  cannot process client-side stoploss of pseudo ticket #"+ orders.ticket[i], ERR_RUNTIME_ERROR));
         if (orders.type[i] == OP_UNDEFINED) return(!catch("ProcessLocalLimits(9)  #"+ orders.ticket[i] +" with client-side stop-loss still marked as pending", ERR_ILLEGAL_STATE));
         if (orders.closeTime[i] != 0)       return(!catch("ProcessLocalLimits(10)  #"+ orders.ticket[i] +" with client-side stop-loss already marked as closed", ERR_ILLEGAL_STATE));
...
bool Grid.SetData(int hSeq, int offset, int ticket, int level, double gridbase, int pendingType, datetime pendingTime, double pendingPrice, int type, int openEvent, datetime openTime, double openPrice, int closeEvent, datetime closeTime, double closePrice, double stopLoss, bool clientSL, bool closedBySL, double swap, double commission, double profit) {
   ...
   if (offset < -1)
      return(!catch("Grid.SetData(1)  illegal parameter offset = "+ offset, ERR_INVALID_PARAMETER));
...
bool Grid.InsertElement(int offset) {
   ...
   if (offset < 0 || offset > sizeOfTickets) return(!catch("Grid.InsertElement(1)  illegal parameter offset = "+ offset, ERR_INVALID_PARAMETER));
...
int CreateMagicNumber(int hSeq, int level) {
   if (sequence.id[hSeq] < SID_MIN) return(_EMPTY(catch("CreateMagicNumber(1)  illegal sequence.id = "+ sequence.id[hSeq], ERR_RUNTIME_ERROR)));
...
bool SaveStatus(int hSeq) {
   ...
   if (IsTesting()) /*&&*/ if (!__LOG()) {
      ...
      if (!firstCall) /*&&*/ if (sequence.status[hSeq]!=STATUS_STOPPED) /*&&*/ if (__WHEREAMI__!=CF_DEINIT)
...
bool ValidateConfig(bool interactive) {
   ...
   // (1) GridSize
   if (reasonParameters) {
      if (GridSize != last.GridSize)             return(_false(ValidateConfig.HandleError("ValidateConfig(1)", "Cannot change GridSize of running strategy", interactive)));
      // TODO: Modify ist erlaubt, solange nicht die erste Sequenz gestartet wurde
   }
   if (GridSize < 1)                             return(_false(ValidateConfig.HandleError("ValidateConfig(2)", "Invalid GridSize = "+ GridSize, interactive)));


   // (2) LotSize
   if (reasonParameters) {
      if (NE(LotSize, last.LotSize))             return(_false(ValidateConfig.HandleError("ValidateConfig(3)", "Cannot change LotSize of running strategy", interactive)));
      // TODO: Modify ist erlaubt, solange nicht die erste Sequenz gestartet wurde
   }
   if (LE(LotSize, 0))                           return(_false(ValidateConfig.HandleError("ValidateConfig(4)", "Invalid LotSize = "+ NumberToStr(LotSize, ".+"), interactive)));
   double minLot  = MarketInfo(Symbol(), MODE_MINLOT );
   double maxLot  = MarketInfo(Symbol(), MODE_MAXLOT );
   double lotStep = MarketInfo(Symbol(), MODE_LOTSTEP);
   int error = GetLastError();
   if (IsError(error))                           return(_false(catch("ValidateConfig(5)  symbol=\""+ Symbol() +"\"", error)));
   if (LT(LotSize, minLot))                      return(_false(ValidateConfig.HandleError("ValidateConfig(6)", "Invalid LotSize = "+ NumberToStr(LotSize, ".+") +" (MinLot="+  NumberToStr(minLot, ".+" ) +")", interactive)));
   if (GT(LotSize, maxLot))                      return(_false(ValidateConfig.HandleError("ValidateConfig(7)", "Invalid LotSize = "+ NumberToStr(LotSize, ".+") +" (MaxLot="+  NumberToStr(maxLot, ".+" ) +")", interactive)));
   if (MathModFix(LotSize, lotStep) != 0)        return(_false(ValidateConfig.HandleError("ValidateConfig(8)", "Invalid LotSize = "+ NumberToStr(LotSize, ".+") +" (LotStep="+ NumberToStr(lotStep, ".+") +")", interactive)));
   SS.LotSize();
   ...
   if (!reasonParameters || StartConditions!=last.StartConditions) {
      ...
      switch (start.trend.timeframe) {           // Timeframes > H1 auf H1 umrechnen, iCustom() soll mit maximal PERIOD_H1 geladen werden
         ...
         case PERIOD_MN1:
         case PERIOD_Q1 :                        return(_false(ValidateConfig.HandleError("ValidateConfig(24)", "Invalid StartConditions = \""+ StartConditions +"\"", interactive)));
      }
   ...
   return(!last_error|catch("ValidateConfig(32)"));
}
