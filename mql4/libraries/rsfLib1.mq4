...
/**
 * Handler for order related errors which occurred in the library's order functions. The execution flags passed to the order
 * function determine whether an error is filtered and silently set or if it causes a terminating ERROR alert. Filtered
 ...
 * @param  int    filter        - filter flags specifying the errors to set silently
...
int Order.HandleError(string message, int error, bool terminalError, int filter, int oe[]) {
   ...
   // if the error was generated by the terminal update current market prices in struct ORDER_EXECUTION
   if (terminalError) {
      switch (error) {
...
bool OrderCloseByEx(int ticket, int opposite, color markerColor, int oeFlags, int oe[]) {
   ...
   if (OrderCloseTime() != 0)                                     return(_false(Order.HandleError("OrderCloseByEx(3)  ticket #"+ ticket +" is already closed", ERR_INVALID_TRADE_PARAMETERS, false, oeFlags, oe), OrderPop("OrderCloseByEx(4)")));
   if (OrderType() > OP_SELL)                                     return(_false(Order.HandleError("OrderCloseByEx(5)  ticket #"+ ticket +" is not an open position: "+ OperationTypeDescription(OrderType()), ERR_INVALID_TRADE_PARAMETERS, false, oeFlags, oe), OrderPop("OrderCloseByEx(6)")));
   ...
   if (symbol != OrderSymbol())                                   return(_false(Order.HandleError("OrderCloseByEx(8)  ticket #"+ opposite +" ("+ OperationTypeDescription(OrderType()) +" "+ OrderSymbol() +") is not opposite to ticket #"+ ticket +" ("+ OperationTypeDescription(ticketType) +" "+ symbol +")", ERR_MULTIPLE_SYMBOLS, false, oeFlags, oe), OrderPop("OrderCloseByEx(9)")));
   if (OrderCloseTime() != 0)                                     return(_false(Order.HandleError("OrderCloseByEx(10)  opposite ticket #"+ opposite +" is already closed", ERR_INVALID_TRADE_PARAMETERS, false, oeFlags, oe), OrderPop("OrderCloseByEx(11)")));
   int      oppositeType     = OrderType();
   double   oppositeLots     = OrderLots();
   datetime oppositeOpenTime = OrderOpenTime();
   if (ticketType != oppositeType^1)                              return(_false(Order.HandleError("OrderCloseByEx(12)  ticket #"+ opposite +" ("+ OperationTypeDescription(oppositeType) +") is not opposite to ticket #"+ ticket +" ("+ OperationTypeDescription(ticketType) +")", ERR_INVALID_TRADE_PARAMETERS, false, oeFlags, oe), OrderPop("OrderCloseByEx(13)")));
   // markerColor
   if (markerColor < CLR_NONE || markerColor > C'255,255,255')    return(_false(Order.HandleError("OrderCloseByEx(14)  illegal parameter markerColor = 0x"+ IntToHexStr(markerColor), ERR_INVALID_PARAMETER, false, oeFlags, oe), OrderPop("OrderCloseByEx(15)")));
   ...
   */
   ...
   while (true) {
      if (IsStopped()) return(_false(Order.HandleError("OrderCloseByEx(16)  "+ OrderCloseByEx.ErrorMsg(first, second, oe), ERS_EXECUTION_STOPPING, false, oeFlags, oe), OrderPop("OrderCloseByEx(17)")));
      ...
      if (success) {
         ...
         if (!EQ(firstLots, secondLots)) {
            ...
            if (smallerByLarger) {                                               // online
               ...
               if (!remainder) return(_false(Order.HandleError("OrderCloseByEx(19)  cannot find remaining position of close #"+ ticket +" ("+ NumberToStr(ticketLots, ".+") +" lots = smaller) by #"+ opposite +" ("+ NumberToStr(oppositeLots, ".+") +" lots = larger)", ERR_RUNTIME_ERROR, false, oeFlags, oe), OrderPop("OrderCloseByEx(20)")));
            }
               ...
               for (i=OrdersTotal()-1; i >= 0; i--) {
                  if (!OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) return(_false(Order.HandleError("OrderCloseByEx(22)->OrderSelect(i="+ i +", SELECT_BY_POS, MODE_TRADES)  unexpectedly returned FALSE", ERR_RUNTIME_ERROR, false, oeFlags, oe), OrderPop("OrderCloseByEx(23)")));
               ...
               if (!remainder) return(_false(Order.HandleError("OrderCloseByEx(24)  cannot find remaining position of close #"+ ticket +" ("+ NumberToStr(ticketLots, ".+") +" lots = larger) by #"+ opposite +" ("+ NumberToStr(oppositeLots, ".+") +" lots = smaller)", ERR_RUNTIME_ERROR, false, oeFlags, oe), OrderPop("OrderCloseByEx(25)")));
            }
   ...
   return(_false(Order.HandleError("OrderCloseByEx(33)  "+ OrderCloseByEx.ErrorMsg(first, second, oe), error, true, oeFlags, oe), OrderPop("OrderCloseByEx(34)")));
}
...
bool OrdersClose(int tickets[], double slippage, color markerColor, int oeFlags, int oes[][]) {
   ...
   if (sizeOfSymbols == 1) {
      if (!OrdersClose.OneSymbol(tickets, slippage, markerColor, oeFlags, oes2))
 ...
 */
int OrdersHedge(int tickets[], double slippage, int oeFlags, /*ORDER_EXECUTION*/int oes[][]) {
   // keine nochmalige, ausführliche Parametervalidierung (da private)
   int sizeOfTickets = ArraySize(tickets);
   if (sizeOfTickets == 0)
      return(_NULL(oes.setError(oes, -1, catch("OrdersHedge(1)  invalid parameter tickets, size = "+ sizeOfTickets, ERR_INVALID_PARAMETER))));


   // (1) oes[] vorbelegen, dabei Lotsizes und Gesamtposition ermitteln
   ArrayResize(oes, sizeOfTickets); ArrayInitialize(oes, 0);
   if (!SelectTicket(tickets[0], "OrdersHedge(2)", O_PUSH))
   ...
   for (int i=0; i < sizeOfTickets; i++) {
      if (!SelectTicket(tickets[i], "OrdersHedge(3)", NULL, O_POP))
         return(_NULL(oes.setError(oes, -1, last_error)));
   ...
   if (EQ(totalLots, 0)) {
      if (__LOG()) log(StringConcatenate("OrdersHedge(4)  ", sizeOfTickets, " ", symbol, " positions ", TicketsToStr.Lots(tickets, NULL), " are already flat"));
      ...
      if (!SelectTicket(tickets.copy[sizeOfTickets-1], "OrdersHedge(5)", NULL, O_POP))
         return(_NULL(oes.setError(oes, -1, last_error)));
      ...
      if (!OrderPop("OrdersHedge(6)"))
         return(_NULL(oes.setError(oes, -1, last_error)));
      ArrayResize(tickets.copy, 0);
   }
   else {
      if (!OrderPop("OrdersHedge(7)"))
         return(_NULL(oes.setError(oes, -1, last_error)));
      if (__LOG()) log(StringConcatenate("OrdersHedge(8)  flattening ", sizeOfTickets, " ", symbol, " position", ifString(sizeOfTickets==1, " ", "s "), TicketsToStr.Lots(tickets, NULL)));
...
/**
 * Close multiple order tickets forming a hedged (flat) position. All tickets must belong to the same symbol.
 *
 ...
 *
 *        (2) oe.Swap, oe.Commission and oe.Profit of the single tickets may differ from actual values. However, the summed-up
 *            values of all tickets are consistent with actual total values.
 *
 *        (3) The returned oe.Error field is set to the same value for all tickets. Typical trade operation errors are:
 *            - ERR_INVALID_TICKET:           one of the ids is not a valid ticket id
...
bool OrderCloseHedged(int tickets[], color markerColor, int oeFlags, int oes[][]) {
   ...
   ArrayResize(oes, sizeOfTickets); ArrayInitialize(oes, 0);
   // tickets[]
   if (sizeOfTickets < 2)                                        return(!oes.setError(oes, -1, catch("OrderCloseHedged(3)  invalid parameter tickets (size="+ sizeOfTickets +")", ERR_INVALID_PARAMETER)));
   // markerColor
   if (markerColor < CLR_NONE || markerColor > C'255,255,255')   return(!oes.setError(oes, -1, catch("OrderCloseHedged(4)  illegal parameter markerColor = 0x"+ IntToHexStr(markerColor), ERR_INVALID_PARAMETER)));
   ...
   for (int i=0; i < sizeOfTickets; i++) {
      ...
      if (OrderSymbol() != symbol)                                       return(!oes.setError(oes, -1, catch("OrderCloseHedged(7)  tickets belong to multiple symbols", ERR_MULTIPLE_SYMBOLS, O_POP)));
      oes.setTicket    (oes, i, tickets[i]       );
      oes.setSymbol    (oes, i, symbol           );
      oes.setDigits    (oes, i, digits           );
      oes.setBid       (oes, i, bid              );
      oes.setAsk       (oes, i, ask              );
      if (OrderCloseTime() != 0)                                         return(!oes.setError(oes, -1, catch("OrderCloseHedged(8)  ticket #"+ tickets[i] +" is already closed", ERR_INVALID_TRADE_PARAMETERS, O_POP)));
      if (OrderType() > OP_SELL)                                         return(!oes.setError(oes, -1, catch("OrderCloseHedged(9)  ticket #"+ tickets[i] +" is not an open position", ERR_INVALID_TRADE_PARAMETERS, O_POP)));
      oes.setType      (oes, i, OrderType()      );
      oes.setLots      (oes, i, OrderLots()      );
   ...
   if (NE(lots, 0, 2)) return(!oes.setError(oes, -1, catch("OrderCloseHedged(10)  tickets don't form a flat position (total position: "+ DoubleToStr(lots, 2) +")", ERR_TOTAL_POSITION_NOT_FLAT, O_POP)));

   if (__LOG()) log("OrderCloseHedged(11)  closing "+ sizeOfTickets +" hedged "+ OrderSymbol() +" positions "+ TicketsToStr.Lots(tickets, NULL));
   ...
   if (!SelectTicket(ticketsCopy[sizeOfCopy-1], "OrderCloseHedged(12)", NULL, O_POP))  // das zuletzt geöffnete Ticket
      return(!oes.setError(oes, -1, ERR_INVALID_TICKET));
   for (i=0; i < sizeOfTickets; i++) {
      oes.setCloseTime (oes, i, OrderOpenTime() );
      oes.setClosePrice(oes, i, OrderOpenPrice());
   }
   ...
   while (sizeOfCopy > 0) {
      int opposite, first=ticketsCopy[0];
      if (!SelectTicket(first, "OrderCloseHedged(13)", NULL, O_POP))             return(!oes.setError(oes, -1, ERR_INVALID_TICKET));
      int firstType = OrderType();

      for (i=1; i < sizeOfCopy; i++) {
         if (!SelectTicket(ticketsCopy[i], "OrderCloseHedged(14)", NULL, O_POP)) return(!oes.setError(oes, -1, ERR_INVALID_TICKET));
         if (OrderType() == firstType^1) {
            opposite = ticketsCopy[i];                                                // erste Opposite-Position ermitteln
            break;
         }
      }
      if (!opposite) return(!oes.setError(oes, -1, catch("OrderCloseHedged(15)  cannot find opposite position for "+ OperationTypeDescription(firstType) +" #"+ first, ERR_RUNTIME_ERROR, O_POP)));

      int oe[];
      if (!OrderCloseByEx(first, opposite, markerColor, oeFlags, oe))                  // erste und Opposite-Position schließen
         return(_false(oes.setError(oes, -1, last_error), OrderPop("OrderCloseHedged(16)")));

      sizeOfCopy -= ArraySpliceInts(ticketsCopy, 0, 1);                                // erstes und opposite Ticket löschen
   ...
   ArrayResize(oe,          0);
   ArrayResize(ticketsCopy, 0);
   return(!oes.setError(oes, -1, catch("OrderCloseHedged(17)", NULL, O_POP)));
}
 ...
 */
bool OrdersClose.OneSymbol(int tickets[], double slippage, color markerColor, int oeFlags, /*ORDER_EXECUTION*/int oes[][]) {
   // keine nochmalige, ausführliche Parametervalidierung (da private)
   int sizeOfTickets = ArraySize(tickets);
   if (sizeOfTickets == 0)
      return(!oes.setError(oes, -1, catch("OrdersClose.OneSymbol(1)  invalid parameter tickets, size = "+ sizeOfTickets, ERR_INVALID_PARAMETER)));
   ArrayResize(oes, sizeOfTickets); ArrayInitialize(oes, 0);
   ...
   if (__LOG()) log(StringConcatenate("OrdersClose.OneSymbol(2)  closing ", sizeOfTickets, " ", OrderSymbol(), " positions ", TicketsToStr.Lots(tickets, NULL)));


   // (2) oes[] vorbelegen
   if (!SelectTicket(tickets[0], "OrdersClose.OneSymbol(3)", O_PUSH))
      return(_false(oes.setError(oes, -1, last_error)));
   int digits = MarketInfo(OrderSymbol(), MODE_DIGITS);

   for (i=0; i < sizeOfTickets; i++) {
      if (!SelectTicket(tickets[i], "OrdersClose.OneSymbol(4)", NULL, O_POP))
         return(_false(oes.setError(oes, -1, last_error)));
      oes.setSymbol    (oes, i, OrderSymbol()    );
      oes.setDigits    (oes, i, digits           );
      oes.setTicket    (oes, i, tickets[i]       );
   ...
   if (IsLastError())                                                                              //  0: Fehler oder Gesamtposition war bereits flat
      return(_false(oes.setError(oes, -1, last_error), OrderPop("OrdersClose.OneSymbol(5)"))); // >0: neues Ticket
   ...
   if (!OrderCloseHedged(tickets.copy, markerColor, oeFlags, oes2))
      return(_false(oes.setError(oes, -1, last_error), OrderPop("OrdersClose.OneSymbol(6)")));
   ...
   ArrayResize(oes2,         0);
   ArrayResize(tickets.copy, 0);
   return(!oes.setError(oes, -1, catch("OrdersClose.OneSymbol(7)", NULL, O_POP)));
}
