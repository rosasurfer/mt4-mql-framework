...
bool Configure.SetParameter(int signal, int timeframe, int lookback, string param, string value) {
   ...
   if (i == size) {
      if (IsLog()) log("Configure.SetParameter(1)  main configuration for signal parameter "+ SignalToStr(signal) +"."+ param +"="+ DoubleQuoteStr(value) +" not found");
      return(true);
   }
   ...
   if (signal == SIGNAL_BAR_CLOSE) {
      ...
      else if (IsLog()) log("Configure.SetParameter(2)  BarClose signal: unknown parameter "+ param +"="+ DoubleQuoteStr(value));
      return(true);
   }
   ...
   if (signal == SIGNAL_BAR_RANGE) {
      ...
      else if (IsLog()) log("Configure.SetParameter(3)  BarRange signal: unknown parameter "+ param +"="+ DoubleQuoteStr(value));
      return(true);
   }
...
bool onBarCloseSignal(int index, int direction) {
   ...
   string message = "";
   if (IsLog()) log("onBarCloseSignal(2)  "+ message);
...
bool onBarRangeSignal(int index, int direction, double level, double price, datetime time.srv) {
   ...
   if (IsLog()) log("onBarRangeSignal(2)  "+ message);
...
int ShowStatus(int error=NULL) {
   ...
   string msg = StringConcatenate(NAME(), sSettings, sError, NL);
