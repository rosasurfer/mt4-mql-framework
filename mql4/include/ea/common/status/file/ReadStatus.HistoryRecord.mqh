...
int ReadStatus.HistoryRecord(string key, string value) {
   ...
   // history.i=ticket,type,lots,openTime,openPrice,openPriceSig,closeTime,closePrice,closePriceSig,slippage,swap,commission,grossProfit,netProfit,netProfitP,runupP,drawdownP,sigProfitP,sigRunupP,sigDrawdownP
   string values[];
   ...
   return(AddHistoryRecord(ticket, type, lots, openTime, openPrice, openPriceSig, closeTime, closePrice, closePriceSig, slippage, swap, commission, grossProfit, netProfit, netProfitP, runupP, drawdownP, sigProfitP, sigRunupP, sigDrawdownP));
}
