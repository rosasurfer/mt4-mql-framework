...
int HistoryFile2.Open(string symbol, int timeframe, string copyright, int digits, int format, int mode, string server="") {
   ...
   if (!read_only) /*&&*/ if (!IsDirectoryA(fullHstDir, MODE_OS)) return(_NULL(catch("HistoryFile2.Open(6)  directory "+ DoubleQuoteStr(fullHstDir) +" doesn't exist ("+ symbol +","+ PeriodDescription(timeframe) +")", ERR_RUNTIME_ERROR)));
