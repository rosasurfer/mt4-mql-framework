   ...
   if (directory == "") {                                               // current trade server: use MQL::FileOpenHistory()
      ...
      for (i=0; i < sizeOfPeriods; i++) {
         ...
         if (IsFile(filename, MODE_SYSTEM)) {                           // reset existing file to 0
            ...
            if (hFile <= 0) return(!catch("HistorySet2.Create(7)->FileOpenHistory(\""+ filename +"\", FILE_WRITE) => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));
...
int HistoryFile2.Open(string symbol, int timeframe, string description, int digits, int format, int mode, string directory = "") {
   ...
   if (directory == "") {                                                        // current trade server: use MQL::FileOpenHistory()
      ...
      if (!read_only) /*&&*/ if (!CreateDirectory(serverPath, MODE_SYSTEM|MODE_MKPARENT)) return(!catch("HistoryFile2.Open(6)  cannot create directory "+ DoubleQuoteStr(serverPath) +" ("+ symbol +","+ PeriodDescription(timeframe) +")", ERR_RUNTIME_ERROR));
      ...
      if (read_only) {
         ...
         if (hFile <= 0) return(!catch("HistoryFile2.Open(7)->FileOpenHistory(\""+ basename +"\", FILE_READ) => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
      // read-write
      else if (read_write) {
         hFile = FileOpenHistory(basename, mode|FILE_BIN);
         if (hFile <= 0) return(!catch("HistoryFile2.Open(8)->FileOpenHistory(\""+ basename +"\", FILE_READ|FILE_WRITE) => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
      // write-only
      else if (write_only) {
         hFile = FileOpenHistory(basename, mode|FILE_BIN);
         if (hFile <= 0) return(!catch("HistoryFile2.Open(9)->FileOpenHistory(\""+ basename +"\", FILE_WRITE) => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
   }

   else if (!IsAbsolutePath(directory)) {                                        // relative sandbox path: use MQL::FileOpen()
      // on write access make sure the directory exists
      if (!read_only) /*&&*/ if (!CreateDirectory(directory, MODE_MQL|MODE_MKPARENT)) return(!catch("HistoryFile2.Open(10)  cannot create directory "+ DoubleQuoteStr(directory) +" ("+ symbol +","+ PeriodDescription(timeframe) +")", ERR_RUNTIME_ERROR));
