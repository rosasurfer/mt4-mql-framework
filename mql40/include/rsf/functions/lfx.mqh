...
/**
 * Gibt den gr��ten existierenden Marker der offenen Orders des angegebenen Symbols zurück.
 *
 ...
 * @param  _In_     int       i        - Index der zu pr�fenden Order innerhalb des übergebenen LFX_ORDER-Arrays
 ...
 *
 * Nachdem ein Limit getriggert wurde, wird bis zum Eintreffen der Ausf�hrungsbest�tigung derselbe Triggerstatus zurückgegeben.
 */
...
bool LFX.SendTradeCommand(/*LFX_ORDER*/int orders[][], int i, int limitType) {
   ...
   if (!triggerTime) {
      ...
      // (3) Die Orderausf�hrung wurde eingeleitet und die Ausf�hrungsbest�tigung ist überf�llig.
   }
   */
...
int LFX.GetOrder(int ticket, /*LFX_ORDER*/int lo[]) {
   ...
   // (3) Orderdaten in übergebenes Array schreiben (erst nach vollst�ndiger erfolgreicher Validierung)
...
int LFX.GetOrders(string currency, int fSelection, /*LFX_ORDER*/int orders[][]) {
   ...
   for (int i=0; i < keysSize; i++) {
      ...
      if (match)
         ArrayPushInts(orders, order);                                     // bei Match Order an übergebenes LFX_ORDER-Array anf�gen
   }
   ArrayResize(keys,  0);
   ArrayResize(order, 0);
...
/**
 * Startet einen QuickChannel-Sender f�r "TradeToLfxTerminal"-Messages. Das LFX-Terminal kann sich über diesen Channel auch selbst
 * Messages schicken.
 *
