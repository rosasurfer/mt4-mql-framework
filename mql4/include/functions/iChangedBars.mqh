   ...
   */
   ...
   if (bars > 0) {
      ...
      if (!data[i][CB.Tick]) {                                                   // first call for the timeseries
         ...
         debug("iChangedBars(4)  number of bars unchanged but last bars differs, hit series MAX_CHART_BARS? (bars="+ bars +", lastBar="+ TimeToStr(lastBarTime, TIME_FULL) +", prevLastBar="+ TimeToStr(data[i][CB.LastBarTime], TIME_FULL) +")");
