 ...
 */
int HistorySet2.Create(string symbol, string descr, int digits, int format, string directory = "") {
   // validate parameters
   ...
   if (StringLen(descr) > 63) {
      logNotice("HistorySet2.Create(4)  truncating too long history description "+ DoubleQuoteStr(descr) +" to 63 chars...");
   ...
   if (directory == "") {                                               // current trade server, use MQL::FileOpenHistory()
      ...
      for (i=0; i < sizeOfPeriods; i++) {
         ...
         if (IsFile(filename, MODE_SYSTEM)) {                           // reset existing file to 0
            ...
            if (hFile <= 0) return(!catch("HistorySet2.Create(7)->FileOpenHistory(\""+ basename +"\", FILE_WRITE) => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));

            hh_SetPeriod(hh, periods[i]);
            FileWriteArray(hFile, hh, 0, ArraySize(hh));                // write new HISTORY_HEADER
            FileClose(hFile);
            error = GetLastError();
            if (error != NO_ERROR) return(!catch("HistorySet2.Create(8)  symbol="+ DoubleQuoteStr(symbol), error));
         }
      }
   }
      ...
      for (i=0; i < sizeOfPeriods; i++) {
         ...
         if (IsFile(filename, MODE_MQL)) {                              // reset existing file to 0
            ...
            if (hFile <= 0) return(!catch("HistorySet2.Create(9)->FileOpen(\""+ filename +"\") => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));

            hh_SetPeriod(hh, periods[i]);
            FileWriteArray(hFile, hh, 0, ArraySize(hh));                // write new HISTORY_HEADER
            FileClose(hFile);
            error = GetLastError();
            if (error != NO_ERROR) return(!catch("HistorySet2.Create(10)  symbol="+ DoubleQuoteStr(symbol), error));
         }
      }
   }

   else {                                                               // absolute path, use Expander
      return(!catch("HistorySet2.Create(11)  accessing absolute path \""+ directory +"\" not yet implemented", ERR_NOT_IMPLEMENTED));
   }
...
int HistoryFile2.Open(string symbol, int timeframe, string description, int digits, int format, int mode, string directory = "") {
   ...
   if (directory == "") {                                                        // current trade server, uses MQL::FileOpenHistory()
      ...
      if (read_only) {
         ...
         if (hFile <= 0) return(!catch("HistoryFile2.Open(6)->FileOpenHistory(\""+ filename +"\", FILE_READ) => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
      // read-write
      else if (read_write) {
         hFile = FileOpenHistory(filename, mode|FILE_BIN);
         if (hFile <= 0) return(!catch("HistoryFile2.Open(7)->FileOpenHistory(\""+ filename +"\", FILE_READ|FILE_WRITE) => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
      // write-only
      else if (write_only) {
         hFile = FileOpenHistory(filename, mode|FILE_BIN);
         if (hFile <= 0) return(!catch("HistoryFile2.Open(8)->FileOpenHistory(\""+ filename +"\", FILE_WRITE) => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
   }
      ...
      if (read_only) {
         ...
         if (hFile <= 0) return(!catch("HistoryFile2.Open(9)->FileOpen(\""+ filename +"\", FILE_READ) => "+ hFile +" ("+ symbol +","+ PeriodDescription(timeframe) +")", intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
      // read-write
      else if (read_write) {
         hFile = FileOpen(filename, mode|FILE_BIN);
         if (hFile <= 0) return(!catch("HistoryFile2.Open(10)->FileOpen(\""+ filename +"\", FILE_READ|FILE_WRITE) => "+ hFile +" ("+ symbol +","+ PeriodDescription(timeframe) +")", intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
      // write-only
      else if (write_only) {
         hFile = FileOpen(filename, mode|FILE_BIN);
         if (hFile <= 0) return(!catch("HistoryFile2.Open(11)->FileOpen(\""+ filename +"\", FILE_WRITE) => "+ hFile +" ("+ symbol +","+ PeriodDescription(timeframe) +")", intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
   }

   else {                                                                        // absolute path, uses MT4Expander
      return(!catch("HistoryFile2.Open(12)  accessing absolute path \""+ directory +"\" not yet implemented", ERR_NOT_IMPLEMENTED));
   }
   ...
   if (write_only || (read_write && fileSize < HISTORY_HEADER_size)) {
      ...
      if (digits < 0)                 return(!catch("HistoryFile2.Open(13)  invalid parameter digits: "+ digits +" ("+ symbol +","+ PeriodDescription(timeframe) +")", ERR_INVALID_PARAMETER));
      if (format!=400 && format!=401) return(!catch("HistoryFile2.Open(14)  invalid parameter format: "+ format +" (must be 400 or 401, symbol="+ symbol +","+ PeriodDescription(timeframe) +")", ERR_INVALID_PARAMETER));
      ...
      if (FileReadArray(hFile, hh, 0, HISTORY_HEADER_intSize) != HISTORY_HEADER_intSize) {
         FileClose(hFile);
         return(!catch("HistoryFile2.Open(15)  invalid history file \""+ filename +"\" (size="+ fileSize +")", intOr(GetLastError(), ERR_RUNTIME_ERROR)));
      }
   ...
   return(!catch("HistoryFile2.Open(16)  "+ symbol +","+ PeriodDescription(timeframe), error));
}
...
bool HistoryFile2.AddTick(int hFile, datetime time, double value, int flags = NULL) {
   ...
   if (tick.offset < hf.total.bars[hFile]) {
      ...
      bar[BAR_V] = 1;
      if (!HistoryFile2.InsertBar(hFile, tick.offset, bar, flags)) return(false);
   }
   return(true);
}
