...
int      hs.hSet.lastValid;                           // das letzte g�ltige, offene Handle (um ein übergebenes Handle nicht st�ndig neu validieren zu m�ssen)
...
int      hf.hFile.lastValid;                          // das letzte g�ltige, offene Handle (um ein übergebenes Handle nicht st�ndig neu validieren zu m�ssen)
...
//
// (1) Beim Aktualisieren dieser Bar mit neuen Ticks braucht die Bar nicht jedesmal neu eingelesen werden: siehe HistoryFile3.UpdateBar().
// (2) Bei funktions�bergreifenden Abl�ufen muß diese Bar nicht überall als Parameter durchgeschleift werden (durch unterschiedliche Arraydimensionen schwierig).
// ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//
// (1) Diese Bar stimmt mit hf.lastStoredBar nur dann überein, wenn hf.lastStoredBar die j�ngste Bar der Datei ist und mit HST_BUFFER_TICKS=on weitere Ticks f�r diese
//     j�ngste Bar gepuffert werden. Stimmen beide Bars überein, werden sie bei �nderungen an einer der Bars jeweils synchronisiert.
// ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 */
int HistorySet3.Create(string symbol, string descr, int digits, int format, string directory = "") {
   // validate parameters
   ...
   if (StringLen(descr) > 63) {
      logNotice("HistorySet3.Create(4)  truncating too long history description "+ DoubleQuoteStr(descr) +" to 63 chars...");
...
int HistorySet3.Get(string symbol, string directory = "") {
   if (!StringLen(symbol))                    return(!catch("HistorySet3.Get(1)  invalid parameter symbol: "+ DoubleQuoteStr(symbol), ERR_INVALID_PARAMETER));
   if (StringLen(symbol) > MAX_SYMBOL_LENGTH) return(!catch("HistorySet3.Get(2)  invalid parameter symbol: "+ DoubleQuoteStr(symbol) +" (max "+ MAX_SYMBOL_LENGTH +" chars)", ERR_INVALID_PARAMETER));
   if (StrContains(symbol, " "))              return(!catch("HistorySet3.Get(3)  invalid parameter symbol: "+ DoubleQuoteStr(symbol) +" (must not contain spaces)", ERR_INVALID_PARAMETER));
   string symbolU = StrToUpper(symbol);
   if (directory == "0") directory = "";                             // (string) NULL
   ...
   if (directory == "") {                                            // current trade server, use MQL::FileOpenHistory()
      ...
      for (i=0; i < sizeOfPeriods; i++) {
         ...
         if (IsFile(filename, MODE_SYSTEM)) {                        // without the additional check FileOpenHistory(READ) logs a warning if the file doesn't exist
            ...
            if (fileSize < HISTORY_HEADER_size) {
               FileClose(hFile);
               logWarn("HistorySet3.Get(5)  invalid history file \""+ filename +"\" (size="+ fileSize +", too small), skipping...");
               continue;
            }
      ...
      for (i=0; i < sizeOfPeriods; i++) {
         ...
         if (IsFile(filename, MODE_MQL)) {                           // without the additional check FileOpen(READ) logs a warning if the file doesn't exist
            ...
            if (hFile <= 0) return(!catch("HistorySet3.Get(6)  hFile(\""+ filename +"\") => "+ hFile, intOr(GetLastError(), ERR_RUNTIME_ERROR)));

            fileSize = FileSize(hFile);
            if (fileSize < HISTORY_HEADER_size) {
               FileClose(hFile);
               logWarn("HistorySet3.Get(7)  invalid history file \""+ filename +"\" (size="+ fileSize +", too small), skipping...");
               continue;
            }
      ...
      return(!catch("HistorySet3.Get(8)  accessing absolute path \""+ directory +"\" not yet implemented", ERR_NOT_IMPLEMENTED));
   }

   int error = GetLastError();
   if (!error) return(-1);
   return(!catch("HistorySet3.Get(9)  symbol=\""+ symbol +"\""));
}
...
bool HistorySet3.Close(int hSet) {
   ...
   for (int i=0; i < sizeOfPeriods; i++) {
      if (hs.hFile[hSet][i] > 0) {                                   // close all open HistoryFiles
         if (!HistoryFile3.Close(hs.hFile[hSet][i])) {
            result = false;
         }
         hs.hFile[hSet][i] = -1;
      }
...
bool HistorySet3.AddTick(int hSet, datetime time, double value, int flags = NULL) {
   ...
   for (int i=0; i < sizeOfPeriods; i++) {
      ...
      if (!HistoryFile3.AddTick(hFile, time, value, flags)) return(false);
   }
   return(true);
 ...
 *
 * @param  int hFile - file handle as returned by HistoryFile3.Open()
 *
 * @return bool - success status
 */
bool HistoryFile3.Close(int hFile) {
   ...
   if (hFile != hf.hFile.lastValid) {
      if (hFile >= ArraySize(hf.hFile)) return(!catch("HistoryFile3.Close(2)  unknown file handle: "+ hFile, ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] == 0)         return(!catch("HistoryFile3.Close(3)  unknown file handle: "+ hFile +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   }
   ...
   // save pending data
   if (hf.bufferedBar.offset[hFile] != -1) if (!HistoryFile3.WriteBufferedBar(hFile)) return(false);
   ...
   if (IsError(error)) return(!catch("HistoryFile3.Close(4)  "+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]), error));
   ...
   return(!catch("HistoryFile3.Close(5)  "+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]), error));
}
 ...
 *                                         � TRUE:  Bar existiert          @see  HistoryFile3.UpdateBar() und HistoryFile3.WriteBar()
 *                                         � FALSE: Bar existiert nicht    @see  HistoryFile3.InsertBar()
 *
...
int HistoryFile3.FindBar(int hFile, datetime time, bool &lpBarExists[]) {
   ...
   //       l��t sich über die Metadaten der Historydatei einfacher herausfinden, ob eine Bar an einem Offset existiert oder nicht.
   //       @see  int hf.total.bars[]
   if (hFile <= 0)                      return(_EMPTY(catch("HistoryFile3.FindBar(1)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER)));
   if (hFile != hf.hFile.lastValid) {
      if (hFile >= ArraySize(hf.hFile)) return(_EMPTY(catch("HistoryFile3.FindBar(2)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER)));
      if (hf.hFile[hFile] == 0)         return(_EMPTY(catch("HistoryFile3.FindBar(3)  invalid parameter hFile: "+ hFile +" (unknown handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER)));
      if (hf.hFile[hFile] <  0)         return(_EMPTY(catch("HistoryFile3.FindBar(4)  invalid parameter hFile: "+ hFile +" (closed handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER)));
   ...
   return(_EMPTY(catch("HistoryFile3.FindBar(6)  bars="+ hf.total.bars[hFile] +", from='"+ TimeToStr(hf.total.from.openTime[hFile], TIME_FULL) +"', to='"+ TimeToStr(hf.total.to.openTime[hFile], TIME_FULL) +"')  time look-up in a timeseries not yet implemented ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_NOT_IMPLEMENTED)));
}
...
bool HistoryFile3.ReadBar(int hFile, int offset, double &bar[]) {
   ...
   if (hFile != hf.hFile.lastValid) {
      if (hFile >= ArraySize(hf.hFile)) return(!catch("HistoryFile3.ReadBar(2)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] == 0)         return(!catch("HistoryFile3.ReadBar(3)  invalid parameter hFile: "+ hFile +" (unknown handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] <  0)         return(!catch("HistoryFile3.ReadBar(4)  invalid parameter hFile: "+ hFile +" (closed handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      hf.hFile.lastValid = hFile;
   }
   if (offset < 0)                      return(!catch("HistoryFile3.ReadBar(5)  invalid parameter offset: "+ offset +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   if (offset >= hf.total.bars[hFile])  return(!catch("HistoryFile3.ReadBar(6)  invalid parameter offset: "+ offset +" ("+ hf.total.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   ...
   if (!FileSeek(hFile, position, SEEK_SET)) return(!catch("HistoryFile3.ReadBar(7)  "+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile])));
   ...
   datetime openTime = bar[BAR_T]; if (!openTime) return(!catch("HistoryFile3.ReadBar(8)  invalid bar["+ offset +"].time: "+ openTime +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_RUNTIME_ERROR));
   int      V        = bar[BAR_V]; if (!V)        return(!catch("HistoryFile3.ReadBar(9)  invalid bar["+ offset +"].volume: "+ V +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_RUNTIME_ERROR));
   ...
   if (hf.period[hFile] <= PERIOD_W1) {
      ...
      nextCloseTime = DateTime1(TimeYearEx(openTime), TimeMonth(openTime)+2);    // 00:00, 1. des übern�chsten Monats
   }
   ...
   if (!error)
      return(true);
   return(!catch("HistoryFile3.ReadBar(10)  "+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]), error));
}


/**
 * Schreibt eine Bar am angegebenen Offset einer Historydatei. Eine dort vorhandene Bar wird überschrieben. Ist die Bar noch
 * nicht vorhanden, muß ihr Offset an die vorhandenen Bars anschlie�en. Sie darf kein Gap verursachen.
 *
 ...
 *       überpr�ft, ob die Bar-Time eine normalisierte OpenTime f�r den Timeframe der Historydatei ist.
 */
bool HistoryFile3.WriteBar(int hFile, int offset, double bar[], int flags = NULL) {
   ...
   if (hFile != hf.hFile.lastValid) {
      ...
      if (hf.hFile[hFile] == 0)         return(!catch("HistoryFile3.WriteBar(3)  invalid parameter hFile: "+ hFile +" (unknown handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] <  0)         return(!catch("HistoryFile3.WriteBar(4)  invalid parameter hFile: "+ hFile +" (closed handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      hf.hFile.lastValid = hFile;
   }
   if (offset < 0)                      return(!catch("HistoryFile3.WriteBar(5)  invalid parameter offset: "+ offset +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   if (offset > hf.total.bars[hFile])   return(!catch("HistoryFile3.WriteBar(6)  invalid parameter offset: "+ offset +" ("+ hf.total.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   if (ArraySize(bar) != 6)             return(!catch("HistoryFile3.WriteBar(7)  invalid size of parameter bar[]: "+ ArraySize(bar) +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INCOMPATIBLE_ARRAY));

   // Bar validieren
   datetime openTime = Round(bar[BAR_T]); if (!openTime) return(!catch("HistoryFile3.WriteBar(8)  invalid bar["+ offset +"].time: "+ openTime +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   int      V        = Round(bar[BAR_V]); if (!V)        return(!catch("HistoryFile3.WriteBar(9)  invalid bar["+ offset +"].volume: "+ V +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));

   // Sicherstellen, da� bekannte Bars nicht mit einer anderen Bar überschrieben werden               // TODO: if-Tests reduzieren
   if (offset==hf.stored.from.offset  [hFile]) /*&&*/ if (openTime!=hf.stored.from.openTime  [hFile]) return(!catch("HistoryFile3.WriteBar(10)  bar["+ offset +"].time="+ TimeToStr(openTime, TIME_FULL) +" collides with hf.stored.from.time="                                        + TimeToStr(hf.stored.from.openTime  [hFile], TIME_FULL) +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_ILLEGAL_STATE));
   if (offset==hf.stored.to.offset    [hFile]) /*&&*/ if (openTime!=hf.stored.to.openTime    [hFile]) return(!catch("HistoryFile3.WriteBar(11)  bar["+ offset +"].time="+ TimeToStr(openTime, TIME_FULL) +" collides with hf.stored.to.time="                                          + TimeToStr(hf.stored.to.openTime    [hFile], TIME_FULL) +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_ILLEGAL_STATE));
   if (offset==hf.total.to.offset     [hFile]) /*&&*/ if (openTime!=hf.total.to.openTime     [hFile]) return(!catch("HistoryFile3.WriteBar(12)  bar["+ offset +"].time="+ TimeToStr(openTime, TIME_FULL) +" collides with hf.total.to.time="                                           + TimeToStr(hf.total.to.openTime     [hFile], TIME_FULL) +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_ILLEGAL_STATE));
   if (offset==hf.lastStoredBar.offset[hFile]) /*&&*/ if (openTime!=hf.lastStoredBar.openTime[hFile]) return(!catch("HistoryFile3.WriteBar(13)  bar["+ offset +"].time="+ TimeToStr(openTime, TIME_FULL) +" collides with hf.lastStoredBar["+ hf.lastStoredBar.offset[hFile] +"].time="+ TimeToStr(hf.lastStoredBar.openTime[hFile], TIME_FULL) +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_ILLEGAL_STATE));
   ...
   if (hf.bufferedBar.offset[hFile] >= 0) /*&&*/ if (offset > hf.bufferedBar.offset[hFile]) {
      if (!HistoryFile3.WriteBufferedBar(hFile, flags)) return(false);
   ...
   int position = HISTORY_HEADER_size + offset*hf.barSize[hFile], digits=hf.digits[hFile];
   if (!FileSeek(hFile, position, SEEK_SET)) return(!catch("HistoryFile3.WriteBar(14)  "+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile])));
   ...
   //                            mit HistoryFile3.InsertBar() eingef�gt worden sein, das die entsprechende L�cke zwischen beiden Bars einrichtet.
   ...
   int error = GetLastError();
   if (!error) return(true);
   return(!catch("HistoryFile3.WriteBar(15)  "+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]), error));
}


/**
 * Aktualisiert den Schlu�kurs der Bar am angegebenen Offset einer Historydatei. Die Bar muß existieren, entweder in der
...
bool HistoryFile3.UpdateBar(int hFile, int offset, double value) {
   ...
   if (hFile != hf.hFile.lastValid) {
      if (hFile >= ArraySize(hf.hFile)) return(!catch("HistoryFile3.UpdateBar(2)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] == 0)         return(!catch("HistoryFile3.UpdateBar(3)  invalid parameter hFile: "+ hFile +" (unknown handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] <  0)         return(!catch("HistoryFile3.UpdateBar(4)  invalid parameter hFile: "+ hFile +" (closed handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      hf.hFile.lastValid = hFile;
   }
   if (offset < 0 )                     return(!catch("HistoryFile3.UpdateBar(5)  invalid parameter offset: "+ offset +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   if (offset >= hf.total.bars[hFile])  return(!catch("HistoryFile3.UpdateBar(6)  invalid parameter offset: "+ offset +" ("+ hf.total.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   ...
   if (offset == hf.bufferedBar.offset[hFile]) {                                 // BufferedBar
      ...
      return(HistoryFile3.WriteBufferedBar(hFile));
   }
   ...
   if (offset != hf.lastStoredBar.offset[hFile]) {
      ...
      if (!HistoryFile3.ReadBar(hFile, offset, bar)) return(false);              // setzt LastStoredBar auf die gelesene Bar
   }
   ...
   return(HistoryFile3.WriteLastStoredBar(hFile));
}


/**
 * F�gt eine Bar am angegebenen Offset einer Historydatei ein. Eine dort vorhandene Bar wird nicht überschrieben, stattdessen
 * werden die vorhandene und alle folgenden Bars um eine Position nach vorn verschoben. Ist die einzufügende Bar die j�ngste
 * Bar, muß ihr Offset an die vorhandenen Bars genau anschlie�en. Sie darf kein Gap verursachen.
 *
 * @param  int    hFile  - Handle der Historydatei
 * @param  int    offset - Offset der einzufügenden Bar relativ zum Dateiheader (Offset 0 ist die �lteste Bar)
...
bool HistoryFile3.InsertBar(int hFile, int offset, double bar[], int flags = NULL) {
   if (hFile <= 0)                      return(!catch("HistoryFile3.InsertBar(1)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER));
   if (hFile != hf.hFile.lastValid) {
      if (hFile >= ArraySize(hf.hFile)) return(!catch("HistoryFile3.InsertBar(2)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] == 0)         return(!catch("HistoryFile3.InsertBar(3)  invalid parameter hFile: "+ hFile +" (unknown handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] <  0)         return(!catch("HistoryFile3.InsertBar(4)  invalid parameter hFile: "+ hFile +" (closed handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      hf.hFile.lastValid = hFile;
   }
   if (offset < 0)                      return(!catch("HistoryFile3.InsertBar(5)  invalid parameter offset: "+ offset +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   if (offset > hf.total.bars[hFile])   return(!catch("HistoryFile3.InsertBar(6)  invalid parameter offset: "+ offset +" ("+ hf.total.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   ...
   // ggf. L�cke f�r einzufügende Bar schaffen
   if (offset < hf.total.bars[hFile]) {
      if (!HistoryFile3.MoveBars(hFile, offset, offset+1)) return(false);
   }

   // Bar schreiben, HistoryFile3.WriteBar() f�hrt u.a. folgende Tasks aus: - validiert die Bar
 ...
 * @access private
 */
bool HistoryFile3.WriteLastStoredBar(int hFile, int flags = NULL) {
   if (hFile <= 0)                      return(!catch("HistoryFile3.WriteLastStoredBar(1)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER));
   if (hFile != hf.hFile.lastValid) {
      if (hFile >= ArraySize(hf.hFile)) return(!catch("HistoryFile3.WriteLastStoredBar(2)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] == 0)         return(!catch("HistoryFile3.WriteLastStoredBar(3)  invalid parameter hFile: "+ hFile +" (unknown handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] <  0)         return(!catch("HistoryFile3.WriteLastStoredBar(4)  invalid parameter hFile: "+ hFile +" (closed handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      hf.hFile.lastValid = hFile;
   }
   int offset = hf.lastStoredBar.offset[hFile];
   if (offset < 0)                      return(_true(logWarn("HistoryFile3.WriteLastStoredBar(5)  undefined lastStoredBar: hf.lastStoredBar.offset="+ offset +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")")));
   if (offset >= hf.stored.bars[hFile]) return(!catch("HistoryFile3.WriteLastStoredBar(6)  invalid hf.lastStoredBar.offset: "+ offset +" ("+ hf.stored.bars[hFile] +" stored bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));

   // Bar validieren
   datetime openTime = hf.lastStoredBar.openTime[hFile];         if (!openTime) return(!catch("HistoryFile3.WriteLastStoredBar(8)  invalid hf.lastStoredBar["+ offset +"].time: "+ openTime +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_RUNTIME_ERROR));
   int      V  = Round(hf.lastStoredBar.data    [hFile][BAR_V]); if (!V)        return(!catch("HistoryFile3.WriteLastStoredBar(9)  invalid hf.lastStoredBar["+ offset +"].volume: "+ V +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_RUNTIME_ERROR));
   ...
   int error = GetLastError();
   if (!error) return(true);
   return(!catch("HistoryFile3.WriteLastStoredBar(8)  "+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]), error));
}


/**
 * Schreibt den Inhalt der BufferedBar in die Historydatei. Sie ist immer die j�ngste Bar und kann in der History bereits
 ...
 * @access private
 */
bool HistoryFile3.WriteBufferedBar(int hFile, int flags = NULL) {
   if (hFile <= 0)                       return(!catch("HistoryFile3.WriteBufferedBar(1)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER));
   if (hFile != hf.hFile.lastValid) {
      if (hFile >= ArraySize(hf.hFile))  return(!catch("HistoryFile3.WriteBufferedBar(2)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] == 0)          return(!catch("HistoryFile3.WriteBufferedBar(3)  invalid parameter hFile: "+ hFile +" (unknown handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] <  0)          return(!catch("HistoryFile3.WriteBufferedBar(4)  invalid parameter hFile: "+ hFile +" (closed handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      hf.hFile.lastValid = hFile;
   }
   int offset = hf.bufferedBar.offset[hFile];
   if (offset < 0)                       return(_true(logWarn("HistoryFile3.WriteBufferedBar(5)  undefined bufferedBar: hf.bufferedBar.offset="+ offset +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")")));
   if (offset != hf.total.bars[hFile]-1) return(!catch("HistoryFile3.WriteBufferedBar(6)  invalid hf.bufferedBar.offset: "+ offset +" ("+ hf.total.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_RUNTIME_ERROR));
   ...
   if (hf.bufferedBar.modified[hFile]) {
      ...
      // FilePointer positionieren, Daten normalisieren und schreiben
      int position = HISTORY_HEADER_size + offset*hf.barSize[hFile], digits=hf.digits[hFile];
      if (!FileSeek(hFile, position, SEEK_SET)) return(!catch("HistoryFile3.WriteBufferedBar(9)  "+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile])));
   ...
   int error = GetLastError();
   if (!error) return(true);
   return(!catch("HistoryFile3.WriteBufferedBar(10)  "+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]), error));
}
...
bool HistoryFile3.AddTick(int hFile, datetime time, double value, int flags = NULL) {
   ...
   if (hFile != hf.hFile.lastValid) {
      if (hFile >= ArraySize(hf.hFile))    return(!catch("HistoryFile3.AddTick(2)  invalid parameter hFile: "+ hFile, ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] == 0)            return(!catch("HistoryFile3.AddTick(3)  invalid parameter hFile: "+ hFile +" (unknown handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      if (hf.hFile[hFile] <  0)            return(!catch("HistoryFile3.AddTick(4)  invalid parameter hFile: "+ hFile +" (closed handle, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
      hf.hFile.lastValid = hFile;
   }
   ...
   // Danach LastStoredBar benutzen (bevor diese ggf. von HistoryFile3._WriteBufferedBar() überschrieben wird).
   ...
   if (HST_BUFFER_TICKS & flags && 1) {
      ...
      if (hf.bufferedBar.offset[hFile] < 0) {                                          // BufferedBar ist leer
         if (tick.offset == -1) {
            ...
            tick.offset = HistoryFile3.FindBar(hFile, tick.openTime, barExists); if (tick.offset < 0) return(false);
         }
         if (tick.offset < hf.total.bars[hFile]) {                                     // Tickbar existiert, laden
            if (!HistoryFile3.ReadBar(hFile, tick.offset, bar)) return(false);         // ReadBar() setzt LastStoredBar auf die Tickbar
            ...
            if (hf.period[hFile] <= PERIOD_W1) {
               ...
               nextCloseTime = DateTime1(TimeYearEx(tick.openTime), TimeMonth(tick.openTime) + 2); // 00:00, 1. des übern�chsten Monats
            }
   ...
   if (tick.offset < hf.total.bars[hFile]) {
      if (!HistoryFile3.UpdateBar(hFile, tick.offset, tick.value)) return(false);      // existierende Bar aktualisieren
   }
   else {
...
bool CheckOpenHistorySets() {
   ...
   for (int i=0; i < size; i++) {
      if (hs.hSet[i] > 0) {
         ...
         if (!HistorySet3.Close(hs.hSet[i])) {
            error = last_error;
         }
      }
   }
