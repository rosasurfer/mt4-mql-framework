/**
 * Return the number of changed bars of the specified timeseries since the last tick.
 * Replacement for cases where IndicatorCounted() is not available. It holds true:
 *
 *   Bars = ValidBars + ChangedBars
 *   ValidBars = IndicatorCounted()
 *   ChangedBars = Bars - ValidBars
 *
 * This function should be used if IndicatorCounted() is not available, e.g. in experts or in indicators with the requested
 * timeframe different from the current timeframe.
 *
...
int iChangedBars(string symbol = "0", int timeframe = NULL) {
   ...
   /*
   - If a timeseries is accessed the first time iBars() typically sets status ERS_HISTORY_UPDATE and new data may
     arrive later.
   - If an empty timeseries is re-accessed before new data has arrived iBars() sets error ERR_SERIES_NOT_AVAILABLE.
     Here the error is suppressed and 0 (no changed bars) is returned.
   - If an empty timeseries is accessed after recompilation or without a server connection no error may be set.
   - Also iBars() doesn't set an error if the timeseries (symbol or timeframe) is unknown.
   */
