...
#define H_CYCLE               0                    // order history indexes
#define H_STARTTIME           1
#define H_STARTPRICE          2
#define H_GRIDBASE            3                    // TODO: reposition gridbase the next time history gets extended
#define H_STOPTIME            4
#define H_STOPPRICE           5
#define H_TOTALPROFIT         6
#define H_MAXPROFIT           7
#define H_MAXDRAWDOWN         8
#define H_TICKET              9
#define H_LEVEL              10
#define H_LOTS               11
#define H_PENDINGTYPE        12
#define H_PENDINGTIME        13
#define H_PENDINGPRICE       14
#define H_OPENTYPE           15
#define H_OPENTIME           16
#define H_OPENPRICE          17
#define H_CLOSETIME          18
#define H_CLOSEPRICE         19
#define H_SWAP               20
#define H_COMMISSION         21
#define H_PROFIT             22
...
int ShowTradeHistory() {
   ...
   for (i=0; i < orders; i++) {
      if (!long.history[i][H_CLOSETIME])               continue;        // skip open tickets     (should never happen)
      if (long.history[i][H_OPENTYPE] == OP_UNDEFINED) continue;        // skip cancelled orders (should never happen)

      sOpenPrice  = NumberToStr(long.history[i][H_OPENPRICE ], PriceFormat);
      sClosePrice = NumberToStr(long.history[i][H_CLOSEPRICE], PriceFormat);
      text        = "Duel.L."+ sequence.id +"."+ NumberToStr(long.history[i][H_LEVEL], "+.");

      // open marker
      openLabel = StringConcatenate("#", _int(long.history[i][H_TICKET]), " buy ", NumberToStr(long.history[i][H_LOTS], ".1+"), " at ", sOpenPrice);
      if (ObjectFind(openLabel) == -1) ObjectCreate(openLabel, OBJ_ARROW, 0, 0, 0);
      ObjectSet    (openLabel, OBJPROP_ARROWCODE, SYMBOL_ORDEROPEN);
      ObjectSet    (openLabel, OBJPROP_COLOR,     CLR_CLOSED_LONG);
      ObjectSet    (openLabel, OBJPROP_TIME1,     long.history[i][H_OPENTIME]);
      ObjectSet    (openLabel, OBJPROP_PRICE1,    long.history[i][H_OPENPRICE]);
      ObjectSetText(openLabel, text);

      // trend line
      lineLabel = StringConcatenate("#", _int(long.history[i][H_TICKET]), " ", sOpenPrice, " -> ", sClosePrice);
      if (ObjectFind(lineLabel) == -1) ObjectCreate(lineLabel, OBJ_TREND, 0, 0, 0, 0, 0);
      ObjectSet(lineLabel, OBJPROP_RAY,    false);
      ObjectSet(lineLabel, OBJPROP_STYLE,  STYLE_DOT);
      ObjectSet(lineLabel, OBJPROP_COLOR,  Blue);
      ObjectSet(lineLabel, OBJPROP_BACK,   true);
      ObjectSet(lineLabel, OBJPROP_TIME1,  long.history[i][H_OPENTIME]);
      ObjectSet(lineLabel, OBJPROP_PRICE1, long.history[i][H_OPENPRICE]);
      ObjectSet(lineLabel, OBJPROP_TIME2,  long.history[i][H_CLOSETIME]);
      ObjectSet(lineLabel, OBJPROP_PRICE2, long.history[i][H_CLOSEPRICE]);
      ...
      ObjectSet    (closeLabel, OBJPROP_ARROWCODE, SYMBOL_ORDERCLOSE);
      ObjectSet    (closeLabel, OBJPROP_COLOR,     CLR_CLOSED);
      ObjectSet    (closeLabel, OBJPROP_TIME1,     long.history[i][H_CLOSETIME]);
      ObjectSet    (closeLabel, OBJPROP_PRICE1,    long.history[i][H_CLOSEPRICE]);
      ObjectSetText(closeLabel, text);
      closedTrades++;
   }
   ...
   for (i=0; i < orders; i++) {
      if (!short.history[i][H_CLOSETIME])               continue;       // skip open tickets     (should never happen)
      if (short.history[i][H_OPENTYPE] == OP_UNDEFINED) continue;       // skip cancelled orders (should never happen)

      sOpenPrice  = NumberToStr(short.history[i][H_OPENPRICE ], PriceFormat);
      sClosePrice = NumberToStr(short.history[i][H_CLOSEPRICE], PriceFormat);
      text        = "Duel.S."+ sequence.id +"."+ NumberToStr(short.history[i][H_LEVEL], "+.");

      // open marker
      openLabel = StringConcatenate("#", _int(short.history[i][H_TICKET]), " buy ", NumberToStr(short.history[i][H_LOTS], ".1+"), " at ", sOpenPrice);
      if (ObjectFind(openLabel) == -1) ObjectCreate(openLabel, OBJ_ARROW, 0, 0, 0);
      ObjectSet    (openLabel, OBJPROP_ARROWCODE, SYMBOL_ORDEROPEN);
      ObjectSet    (openLabel, OBJPROP_COLOR,     CLR_CLOSED_SHORT);
      ObjectSet    (openLabel, OBJPROP_TIME1,     short.history[i][H_OPENTIME]);
      ObjectSet    (openLabel, OBJPROP_PRICE1,    short.history[i][H_OPENPRICE]);
      ObjectSetText(openLabel, text);

      // trend line
      lineLabel = StringConcatenate("#", _int(short.history[i][H_TICKET]), " ", sOpenPrice, " -> ", sClosePrice);
      if (ObjectFind(lineLabel) == -1) ObjectCreate(lineLabel, OBJ_TREND, 0, 0, 0, 0, 0);
      ObjectSet(lineLabel, OBJPROP_RAY,   false);
      ObjectSet(lineLabel, OBJPROP_STYLE, STYLE_DOT);
      ObjectSet(lineLabel, OBJPROP_COLOR, Red);
      ObjectSet(lineLabel, OBJPROP_BACK,  true);
      ObjectSet(lineLabel, OBJPROP_TIME1,  short.history[i][H_OPENTIME]);
      ObjectSet(lineLabel, OBJPROP_PRICE1, short.history[i][H_OPENPRICE]);
      ObjectSet(lineLabel, OBJPROP_TIME2,  short.history[i][H_CLOSETIME]);
      ObjectSet(lineLabel, OBJPROP_PRICE2, short.history[i][H_CLOSEPRICE]);
      ...
      ObjectSet    (closeLabel, OBJPROP_ARROWCODE, SYMBOL_ORDERCLOSE);
      ObjectSet    (closeLabel, OBJPROP_COLOR,     CLR_CLOSED);
      ObjectSet    (closeLabel, OBJPROP_TIME1,     short.history[i][H_CLOSETIME]);
      ObjectSet    (closeLabel, OBJPROP_PRICE1,    short.history[i][H_CLOSEPRICE]);
      ObjectSetText(closeLabel, text);
      closedTrades++;
   }
...
bool RestorePositions(double history[][], double &openPrice) {
   ...
   int lastCycle = history[size-1][H_CYCLE];
   double price=0, lots=0, sumPrice=0, sumLots=0;

   for (int i=0; i < size; i++) {
      if (history[i][H_CYCLE] == lastCycle) {
         int direction = ifInt(history[i][H_OPENTYPE]==OP_BUY, D_LONG, D_SHORT);

         if (!Grid.AddPosition(direction, history[i][H_LEVEL], price, lots)) return(false);
         sumPrice += lots * price;
         sumLots  += lots;
      }
   }
...
int History.AddRecord(int direction, int index, int cycle, double gridbase, datetime startTime, double startPrice, datetime stopTime, double stopPrice, double totalProfit, double maxProfit, double maxDrawdown, int ticket, int level, double lots, int pendingType, datetime pendingTime, double pendingPrice, int openType, datetime openTime, double openPrice, datetime closeTime, double closePrice, double swap, double commission, double profit) {
   ...
   if (direction == D_LONG) {
      ...
      if (long.history[index][H_CYCLE] != 0) return(!catch("History.AddRecord(2)  "+ sequence.name +" invalid parameter index: "+ index +" (cannot overwrite long.history[] record, cycle="+ long.history[index][H_CYCLE] +", ticket #"+ long.history[index][H_TICKET] +")", ERR_INVALID_PARAMETER));

      long.history[index][H_CYCLE       ] = cycle;
      long.history[index][H_STARTTIME   ] = startTime;
      long.history[index][H_STARTPRICE  ] = startPrice;
      long.history[index][H_GRIDBASE    ] = gridbase;
      long.history[index][H_STOPTIME    ] = stopTime;
      long.history[index][H_STOPPRICE   ] = stopPrice;
      long.history[index][H_TOTALPROFIT ] = totalProfit;
      long.history[index][H_MAXPROFIT   ] = maxProfit;
      long.history[index][H_MAXDRAWDOWN ] = maxDrawdown;
      long.history[index][H_TICKET      ] = ticket;
      long.history[index][H_LEVEL       ] = level;
      long.history[index][H_LOTS        ] = lots;
      long.history[index][H_PENDINGTYPE ] = pendingType;
      long.history[index][H_PENDINGTIME ] = pendingTime;
      long.history[index][H_PENDINGPRICE] = pendingPrice;
      long.history[index][H_OPENTYPE    ] = openType;
      long.history[index][H_OPENTIME    ] = openTime;
      long.history[index][H_OPENPRICE   ] = openPrice;
      long.history[index][H_CLOSETIME   ] = closeTime;
      long.history[index][H_CLOSEPRICE  ] = closePrice;
      long.history[index][H_SWAP        ] = swap;
      long.history[index][H_COMMISSION  ] = commission;
      long.history[index][H_PROFIT      ] = profit;
   }
   else if (direction == D_SHORT) {
      size = ArrayRange(short.history, 0);
      if (index >= size) ArrayResize(short.history, index+1);
      if (short.history[index][H_CYCLE] != 0) return(!catch("History.AddRecord(3)  "+ sequence.name +" invalid parameter index: "+ index +" (cannot overwrite short.history[] record, cycle="+ short.history[index][H_CYCLE] +", ticket #"+ short.history[index][H_TICKET] +")", ERR_INVALID_PARAMETER));

      short.history[index][H_CYCLE       ] = cycle;
      short.history[index][H_STARTTIME   ] = startTime;
      short.history[index][H_STARTPRICE  ] = startPrice;
      short.history[index][H_GRIDBASE    ] = gridbase;
      short.history[index][H_STOPTIME    ] = stopTime;
      short.history[index][H_STOPPRICE   ] = stopPrice;
      short.history[index][H_TOTALPROFIT ] = totalProfit;
      short.history[index][H_MAXPROFIT   ] = maxProfit;
      short.history[index][H_MAXDRAWDOWN ] = maxDrawdown;
      short.history[index][H_TICKET      ] = ticket;
      short.history[index][H_LEVEL       ] = level;
      short.history[index][H_LOTS        ] = lots;
      short.history[index][H_PENDINGTYPE ] = pendingType;
      short.history[index][H_PENDINGTIME ] = pendingTime;
      short.history[index][H_PENDINGPRICE] = pendingPrice;
      short.history[index][H_OPENTYPE    ] = openType;
      short.history[index][H_OPENTIME    ] = openTime;
      short.history[index][H_OPENPRICE   ] = openPrice;
      short.history[index][H_CLOSETIME   ] = closeTime;
      short.history[index][H_CLOSEPRICE  ] = closePrice;
      short.history[index][H_SWAP        ] = swap;
      short.history[index][H_COMMISSION  ] = commission;
      short.history[index][H_PROFIT      ] = profit;
   }
...
bool ArchiveStoppedSequence() {
   ...
   if (long.enabled) {
      ...
      for (int i=0; i < ordersSize; i++) {
         long.history[historySize+i][H_CYCLE       ] = sequence.cycle;          // for simplicity sequence data is duplicated
         long.history[historySize+i][H_STARTTIME   ] = sequence.startTime;      //
         long.history[historySize+i][H_STARTPRICE  ] = sequence.startPrice;     //
         long.history[historySize+i][H_GRIDBASE    ] = sequence.gridbase;       //
         long.history[historySize+i][H_STOPTIME    ] = sequence.stopTime;       //
         long.history[historySize+i][H_STOPPRICE   ] = sequence.stopPrice;      //
         long.history[historySize+i][H_TOTALPROFIT ] = sequence.totalPL;        //
         long.history[historySize+i][H_MAXPROFIT   ] = sequence.maxProfit;      //
         long.history[historySize+i][H_MAXDRAWDOWN ] = sequence.maxDrawdown;    //
         long.history[historySize+i][H_TICKET      ] = long.ticket      [i];
         long.history[historySize+i][H_LEVEL       ] = long.level       [i];
         long.history[historySize+i][H_LOTS        ] = long.lots        [i];
         long.history[historySize+i][H_PENDINGTYPE ] = long.pendingType [i];
         long.history[historySize+i][H_PENDINGTIME ] = long.pendingTime [i];
         long.history[historySize+i][H_PENDINGPRICE] = long.pendingPrice[i];
         long.history[historySize+i][H_OPENTYPE    ] = long.openType    [i];
         long.history[historySize+i][H_OPENTIME    ] = long.openTime    [i];
         long.history[historySize+i][H_OPENPRICE   ] = long.openPrice   [i];
         long.history[historySize+i][H_CLOSETIME   ] = long.closeTime   [i];
         long.history[historySize+i][H_CLOSEPRICE  ] = long.closePrice  [i];
         long.history[historySize+i][H_SWAP        ] = long.swap        [i];
         long.history[historySize+i][H_COMMISSION  ] = long.commission  [i];
         long.history[historySize+i][H_PROFIT      ] = long.profit      [i];
      }
      ArrayResize(long.ticket,       0);
      ArrayResize(long.level,        0);
      ArrayResize(long.lots,         0);
   ...
   if (short.enabled) {
      ...
      for (i=0; i < ordersSize; i++) {
         short.history[historySize+i][H_CYCLE       ] = sequence.cycle;          // for simplicity sequence data is duplicated
         short.history[historySize+i][H_STARTTIME   ] = sequence.startTime;      //
         short.history[historySize+i][H_STARTPRICE  ] = sequence.startPrice;     //
         short.history[historySize+i][H_GRIDBASE    ] = sequence.gridbase;       //
         short.history[historySize+i][H_STOPTIME    ] = sequence.stopTime;       //
         short.history[historySize+i][H_STOPPRICE   ] = sequence.stopPrice;      //
         short.history[historySize+i][H_TOTALPROFIT ] = sequence.totalPL;        //
         short.history[historySize+i][H_MAXPROFIT   ] = sequence.maxProfit;      //
         short.history[historySize+i][H_MAXDRAWDOWN ] = sequence.maxDrawdown;    //
         short.history[historySize+i][H_TICKET      ] = short.ticket      [i];
         short.history[historySize+i][H_LEVEL       ] = short.level       [i];
         short.history[historySize+i][H_LOTS        ] = short.lots        [i];
         short.history[historySize+i][H_PENDINGTYPE ] = short.pendingType [i];
         short.history[historySize+i][H_PENDINGTIME ] = short.pendingTime [i];
         short.history[historySize+i][H_PENDINGPRICE] = short.pendingPrice[i];
         short.history[historySize+i][H_OPENTYPE    ] = short.openType    [i];
         short.history[historySize+i][H_OPENTIME    ] = short.openTime    [i];
         short.history[historySize+i][H_OPENPRICE   ] = short.openPrice   [i];
         short.history[historySize+i][H_CLOSETIME   ] = short.closeTime   [i];
         short.history[historySize+i][H_CLOSEPRICE  ] = short.closePrice  [i];
         short.history[historySize+i][H_SWAP        ] = short.swap        [i];
         short.history[historySize+i][H_COMMISSION  ] = short.commission  [i];
         short.history[historySize+i][H_PROFIT      ] = short.profit      [i];
      }
      ArrayResize(short.ticket,       0);
      ArrayResize(short.level,        0);
      ArrayResize(short.lots,         0);
...
string SaveStatus.HistoryToStr(int direction, int index) {
   ...
   if (direction == D_LONG) {
      cycle        = long.history[index][H_CYCLE       ];
      startTime    = long.history[index][H_STARTTIME   ];
      startPrice   = long.history[index][H_STARTPRICE  ];
      gridbase     = long.history[index][H_GRIDBASE    ];
      stopTime     = long.history[index][H_STOPTIME    ];
      stopPrice    = long.history[index][H_STOPPRICE   ];
      totalProfit  = long.history[index][H_TOTALPROFIT ];
      maxProfit    = long.history[index][H_MAXPROFIT   ];
      maxDrawdown  = long.history[index][H_MAXDRAWDOWN ];
      ticket       = long.history[index][H_TICKET      ];
      level        = long.history[index][H_LEVEL       ];
      lots         = long.history[index][H_LOTS        ];
      pendingType  = long.history[index][H_PENDINGTYPE ];
      pendingTime  = long.history[index][H_PENDINGTIME ];
      pendingPrice = long.history[index][H_PENDINGPRICE];
      openType     = long.history[index][H_OPENTYPE    ];
      openTime     = long.history[index][H_OPENTIME    ];
      openPrice    = long.history[index][H_OPENPRICE   ];
      closeTime    = long.history[index][H_CLOSETIME   ];
      closePrice   = long.history[index][H_CLOSEPRICE  ];
      swap         = long.history[index][H_SWAP        ];
      commission   = long.history[index][H_COMMISSION  ];
      profit       = long.history[index][H_PROFIT      ];
   }
   else if (direction == D_SHORT) {
      cycle        = short.history[index][H_CYCLE       ];
      startTime    = short.history[index][H_STARTTIME   ];
      startPrice   = short.history[index][H_STARTPRICE  ];
      gridbase     = short.history[index][H_GRIDBASE    ];
      stopTime     = short.history[index][H_STOPTIME    ];
      stopPrice    = short.history[index][H_STOPPRICE   ];
      totalProfit  = short.history[index][H_TOTALPROFIT ];
      maxProfit    = short.history[index][H_MAXPROFIT   ];
      maxDrawdown  = short.history[index][H_MAXDRAWDOWN ];
      ticket       = short.history[index][H_TICKET      ];
      level        = short.history[index][H_LEVEL       ];
      lots         = short.history[index][H_LOTS        ];
      pendingType  = short.history[index][H_PENDINGTYPE ];
      pendingTime  = short.history[index][H_PENDINGTIME ];
      pendingPrice = short.history[index][H_PENDINGPRICE];
      openType     = short.history[index][H_OPENTYPE    ];
      openTime     = short.history[index][H_OPENTIME    ];
      openPrice    = short.history[index][H_OPENPRICE   ];
      closeTime    = short.history[index][H_CLOSETIME   ];
      closePrice   = short.history[index][H_CLOSEPRICE  ];
      swap         = short.history[index][H_SWAP        ];
      commission   = short.history[index][H_COMMISSION  ];
      profit       = short.history[index][H_PROFIT      ];
   }
...
bool ReadStatus.ParseOrder(string key, string value) {
   ...
   if (pool == MODE_HISTORY) {
      ...
      int      cycle        = StrToInteger(values[H_CYCLE       ]);
      datetime startTime    = StrToInteger(values[H_STARTTIME   ]);
      double   startPrice   =  StrToDouble(values[H_STARTPRICE  ]);
      double   gridbase     =  StrToDouble(values[H_GRIDBASE    ]);
      datetime stopTime     = StrToInteger(values[H_STOPTIME    ]);
      double   stopPrice    =  StrToDouble(values[H_STOPPRICE   ]);
      double   totalProfit  =  StrToDouble(values[H_TOTALPROFIT ]);
      double   maxProfit    =  StrToDouble(values[H_MAXPROFIT   ]);
      double   maxDrawdown  =  StrToDouble(values[H_MAXDRAWDOWN ]);
               ticket       = StrToInteger(values[H_TICKET      ]);
               level        = StrToInteger(values[H_LEVEL       ]);
               lots         =  StrToDouble(values[H_LOTS        ]);
               pendingType  = StrToInteger(values[H_PENDINGTYPE ]);
               pendingTime  = StrToInteger(values[H_PENDINGTIME ]);
               pendingPrice =  StrToDouble(values[H_PENDINGPRICE]);
               openType     = StrToInteger(values[H_OPENTYPE    ]);
               openTime     = StrToInteger(values[H_OPENTIME    ]);
               openPrice    =  StrToDouble(values[H_OPENPRICE   ]);
               closeTime    = StrToInteger(values[H_CLOSETIME   ]);
               closePrice   =  StrToDouble(values[H_CLOSEPRICE  ]);
               swap         =  StrToDouble(values[H_SWAP        ]);
               commission   =  StrToDouble(values[H_COMMISSION  ]);
               profit       =  StrToDouble(values[H_PROFIT      ]);
...
void SS.CycleStats() {
   ...
   for (int i=0; i < size; i++) {
      int cycle = history[i][H_CYCLE];

      if (cycle != lastCycle) {
         double totalPL     = history[i][H_TOTALPROFIT];
         double maxProfit   = history[i][H_MAXPROFIT  ];
         double maxDrawdown = history[i][H_MAXDRAWDOWN];
