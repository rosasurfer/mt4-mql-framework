/**
 * Erzeugt eine neue LFX-SellLimit- oder StopSell-Order. Muß auf dem jeweiligen LFX-Chart ausgef�hrt werden.
 *
...
int limitType;                                                       // OP_SELL_LIMIT | OP_SELL_STOP
...
int onInit() {
   ...
   if      (sValue=="selllimit" || sValue=="limitsell") limitType = OP_SELL_LIMIT;
   else if (sValue=="sellstop"  || sValue=="stopsell" ) limitType = OP_SELL_STOP;
...
int onStart() {
   ...
   if ((limitType==OP_SELL_LIMIT && LimitPrice <= Close[0]) || (limitType==OP_SELL_STOP && LimitPrice >= Close[0])) {
   ...
   /*LFX_ORDER*/int order[]; InitializeByteBuffer(order, LFX_ORDER_size);
      lo.setTicket           (order, LFX.CreateMagicNumber(lfxOrders, lfxCurrency));   // Ticket immer zuerst, damit im Struct Currency-ID und Digits ermittelt werden k�nnen
   ...
   if (executeNow) {
      ...
      // (4) ...oder Benachrichtigung an den Chart schicken und Order best�tigen
