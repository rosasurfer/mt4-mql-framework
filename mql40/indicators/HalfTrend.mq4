...
#include <rsf/functions/iCustom/HalfTrend.mqh>
...
int onInit() {
   ...
   if (Signal.onTrendChange) {
      if (!ConfigureSignalTypes(signalId, Signal.onTrendChange.Types, AutoConfiguration, signal.sound, signal.alert, signal.mail, signal.telegram)) {
         return(catch("onInit(5)  invalid input parameter Signal.onTrendChange.Types: "+ DoubleQuoteStr(Signal.onTrendChange.Types), ERR_INVALID_INPUT_PARAMETER));
      }
   ...
   indicatorName = indicator +"("+ Periods +")";
...
bool onTrendChange(int direction) {
   ...
   string eventName = "rsf::"+ StdSymbol() +","+ sPeriod +"."+ WindowExpertName() +"("+ Periods +").onTrendChange("+ direction +")."+ TimeToStr(Time[0]), propertyName = "";
   string message1  = indicatorName +" turned "+ ifString(direction==MODE_UPTREND, "up", "down") +" (bid: "+ NumberToStr(_Bid, PriceFormat) +")";
