...
// XMT-Scalper v. 2.4.1
// ------------------------------------------------------------------------------------------------
// Cleaned Out The Code Thoroughly To Work On Build 646 mt4 and above
// ------------------------------------------------------------------------------------------------
#property copyright "Copyright 2011 - 2012, Original by Capella of http://www.worldwide-invest.org"
#property copyright "Copyright 2014, Re-Modded By Fredricktroy"
...
int init()
  {
    ...
    VolatilityPercentageLimit = NormalizeDouble(VolatilityPercentageLimit / 100 + 1, Digits);
   VolatilityMultiplier = NormalizeDouble(VolatilityMultiplier / 10, Digits);
   ArrayInitialize(array_spread, 0);
    VolatilityLimit = NormalizeDouble(VolatilityLimit * Point, Digits);
    Commission = sub_normalizebrokerdigits(Commission * Point);
    TrailingStart = NormalizeDouble(TrailingStart * Point, Digits);
    stoplevel = NormalizeDouble(stoplevel * Point, Digits);
    AddPriceGap = NormalizeDouble(AddPriceGap * Point, Digits);
...
void sub_trade()
{
   ...
   double local_skipticks;
    ...
    {
        if(!(OrderSelect(local_loopcount2, SELECT_BY_POS, MODE_TRADES)))continue;
                ...
                while (TRUE)
                {
                    ...
                    if (local_ordertakeprofit < sub_normalizebrokerdigits(local_askpluscommission + TakeProfit * Point + AddPriceGap) && local_askpluscommission + TakeProfit * Point + AddPriceGap - local_ordertakeprofit > TrailingStart)
                    {
                        ...
                        else
                        {
                            ...
                            if (Debug || Verbose)
                                Print ("Order could not be modified because of ", GetLastError());
       ...
       else
        {
          // Unless backtesting, lets send a fake order to check the OrderModify execution time,
            if (IsTesting() == FALSE)
            {
                ...
                execution = GetTickCount() - execution;
                // Delete the order
                if(!(OrderDelete(local_orderticket)))return;
            }
       }
    }
