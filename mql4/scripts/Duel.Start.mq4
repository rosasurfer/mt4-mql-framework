 ...
 * Duel.Start
 *
 * Send a command to a running Duel instance to start the current sequence.
 */
 ...
 */
int onStart() {
   // A running Duel instance maintains a chart object holding the instance id and the current instance status.
   ...
   if (isStartable) {
      ...
      int button = MessageBoxEx(ProgramName(), ifString(IsDemoFix(), "", "- Real Account -\n\n") +"Do you really want to start the Duel instance (sid "+ sid +")?", MB_ICONQUESTION|MB_OKCANCEL);
      if (button != IDOK) return(catch("onStart(1)"));
      SendChartCommand("Duel.command", "start");
   }
   else {
      PlaySoundEx("Windows Chord.wav");
      MessageBoxEx(ProgramName(), "No active Duel instance found.", MB_ICONEXCLAMATION|MB_OK);
   }
