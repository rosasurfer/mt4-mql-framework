...
bool StopSequence() {
   ...
   if (sequence.status!=STATUS_WAITING) /*&&*/ if (sequence.status!=STATUS_PROGRESSING) /*&&*/ if (sequence.status!=STATUS_STOPPING)
      if (!IsTesting() || __WHEREAMI__!=CF_DEINIT || sequence.status!=STATUS_STOPPED) // ggf. wird nach Testende nur aufgeräumt
         return(_false(catch("StopSequence(2)  cannot stop "+ sequenceStatusDescr[sequence.status] +" sequence "+ Sequence.ID, ERR_RUNTIME_ERROR)));
...
bool ResumeSequence() {
   ...
   if (Tick==1) /*&&*/ if (!ConfirmFirstTickTrade("ResumeSequence()", "Do you really want to resume sequence "+ Sequence.ID +" now?"))
      return(!SetLastError(ERR_CANCELLED_BY_USER));
...
bool UpdateStatus(bool &gridChanged, int activatedLimits[]) {
   ...
   for (int i=0; i < sizeOfTickets; i++) {
      if (!orders.closeTime[i]) {                                                // Ticket prüfen, wenn es beim letzten Aufruf offen war
         ...
         // Magic-Tickets #-2 prüfen (wird sofort hier "geschlossen")
...
string UpdateStatus.SLExecuteMsg(int i) {
   ...
   string strPseudo    = ifString(orders.ticket[i]==-2, "magic ticket ", "");
   string strType      = OperationTypeDescription(orders.type[i]);
   string strOpenPrice = NumberToStr(orders.openPrice[i], PriceFormat);
   string strStopSide  = ifString(orders.clientsideLimit[i], "client-side ", "");
...
bool IsWeekendResumeSignal() {
   ...
   if (result) {
      ...
      if (__LOG()) log(StringConcatenate("IsWeekendResumeSignal(2)  sequence "+ Sequence.ID +" weekend stop price \"", NumberToStr(stopPrice, PriceFormat), "\" met"));
      return(true);
   }
   ...
   if (weekend.resume.time <= now) {
      if (__LOG()) log(StringConcatenate("IsWeekendResumeSignal(3)  sequence "+ Sequence.ID +" resume condition '", GmtTimeFormat(weekend.resume.time, "%a, %Y.%m.%d %H:%M:%S"), "' met"));
      return(true);
   }
   return(false);
}
...
bool UpdatePendingOrders() {
   ...
   if (!nextOrderExists) {
      ...
      if (limitOrders > 0) {
         SS.MissedLevels();
         warn("UpdatePendingOrders(3)  opened "+ limitOrders +" "+ ifString(sequence.direction==D_LONG, "Buy", "Sell") +" Limit order"+ ifString(limitOrders==1, "", "s") +" in a fast moving market");
      }
      ordersChanged = true;
   }
...
bool Grid.AddPosition(int level) {
   ...
   if (ticket <= 0) {
      ...
      if (ticket == -1) {
         ...
         warn("Grid.AddPosition(5)  magic ticket #"+ ticket +" opened for spread violation ("+ NumberToStr(oe.Bid(oe), PriceFormat) +"/"+ NumberToStr(oe.Ask(oe), PriceFormat) +") by "+ OperationTypeDescription(orderType) +" at "+ NumberToStr(oe.OpenPrice(oe), PriceFormat) +", sl="+ NumberToStr(oe.StopLoss(oe), PriceFormat) +" (level "+ level +")");
      }
      // StopDistance violated
      else if (ticket == -2) {
         clientsideSL = true;
...
/**
 * Whether the order with the specified index is/was opened as a result of a stop order.
 *
 * @param  int index - order index in the locally held arrays
 *
 * @return bool
 */
...
/**
 * Whether the order with the specified index is/was opened as a result of a limit order.
 *
 * @param  int index - order index in the locally held arrays
 *
 * @return bool
