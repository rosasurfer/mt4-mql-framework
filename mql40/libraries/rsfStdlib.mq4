...
/**
 * Ermittelt Zeitpunkt und Offset des vorherigen und n�chsten DST-Wechsels der angebenen Serverzeit.
 *
 ...
 * @param  _Out_ datetime nextTransition[] - Array zur Aufnahme der n�chsten Transitionsdaten
 *
 * @return bool - success status
 *
 ...
 * und m�ssen nicht umsortiert werden.
 *
...
/**
 * Ob ein Tradeserver-Fehler tempor�r (also vor�bergehend) ist oder nicht. Bei einem vor�bergehenden Fehler *kann* der erneute Versuch, die
 * Order auszuf�hren, erfolgreich sein.
 *
 * @return int - error status
 *
 * @return bool
...
/**
 * Weist einer Position eines zweidimensionalen Integer-Arrays ein anderes Array zu (entspricht array[i] = array[] f�r ein
 * Array von Arrays).
 *
 ...
 * @param  _In_    int  offset  - Position, an dem das Element eingef�gt werden soll
...
int ArrayInsertBool(bool &array[], int offset, bool value) {
   ...
   array[offset] = value;                                                        // L�cke mit einzufügendem Wert f�llen
 ...
 *
 * @param  _InOut_ double array[][] - zu vergr��erndes zwei-dimensionales Ausgangs-Array
 * @param  _In_    int    offset    - Position im Ausgangs-Array, an dem das andere Array eingef�gt werden soll
...
int ArrayInsertBools(bool array[], int offset, bool values[]) {
   ...
   ArrayCopy(array, values, offset);                                             // L�cke mit einzufügenden Werten überschreiben
...
int ArrayInsertString(string &array[], int offset, string value) {
   ...
   // Einf�gen innerhalb des Arrays: ArrayCopy() "zerst�rt" bei String-Arrays den sich überlappenden Bereich, daher zus�tzliche Kopie n�tig
 ...
 * @param  _In_    int    offset   - Position im Ausgangs-Array, an dem die Elemente eingef�gt werden sollen
...
int ArrayInsertStrings(string &array[], int offset, string values[]) {
   ...
   // ArrayCopy() "zerst�rt" bei String-Arrays den sich überlappenden Bereich, wir m�ssen mit einer zus�tzlichen Kopie arbeiten
   string tmp[]; ArrayResize(tmp, 0);
...
/**
 * Gibt die lesbare Version eines Zeichenbuffers zur�ck. <NUL>-Characters (0x00h) werden gestrichelt (�),
 ...
 * @param  _Out_ string results[]        - Zielarray f�r die Teilstrings
...
int Explode(string input, string separator, string &results[], int limit = NULL) {
   // Der Parameter input *k�nnte* eine Referenz auf ein Element des Ergebnisarrays results[] sein, daher erstellen wir
   ...
   if (StrIsNull(input)) {                   // NULL-Pointer
      ...
      if (i == lenInput) {                   // bei abschlie�endem Separator Substrings mit Leerstring beenden
         ArrayResize(results, size+1);
         results[size] = "";                 // TODO: !!! Wechselwirkung zwischen Limit und Separator am Ende überpr�fen
      }
   }
...
/**
 * Findet alle zum angegebenen Muster passenden Dateinamen. Die Pseudo-Verzeichnisse "." und ".." werden nicht ber�cksichtigt.
 *
 ...
 *
 * @return int - Anzahl der gefundenen Eintr�ge oder EMPTY (-1), falls ein Fehler auftrat
 */
 ...
 * @param  int errors - Anzahl der bisher aufgetretenen tempor�ren Fehler
 *
 * @return string
 */
 ...
 * Das Ticket muß w�hrend der Ausf�hrung selektierbar sein.
 *
...
bool ChartMarker.OrderFilled_B(int ticket, int pendingType, double pendingPrice, int digits, color markerColor, double lots, string symbol, datetime openTime, double openPrice, string comment) {
   ...
   if (ObjectFind(label3) != -1) ObjectDelete(label3);                                 // l�schen
...
bool ChartMarker.PositionClosed_B(int ticket, int digits, color markerColor, int type, double lots, string symbol, datetime openTime, double openPrice, datetime closeTime, double closePrice) {
   ...
   if (markerColor == CLR_NONE) {
      if (ObjectFind(label1) != -1) ObjectDelete(label1);                              // l�schen
   }
   ...
   if (ObjectFind(label2) != -1) {
      if (markerColor == CLR_NONE) ObjectDelete(label2);                               // l�schen
   }
...
/**
 * MQL-Wrapper f�r die Win32-Funktion GetTempPath().
 *
 * @return string - Dateipfad f�r tempor�re Dateien oder Leerstring, falls ein Fehler auftrat
 */
string GetTempPath() {
...
int AddSymbolGroup(/*SYMBOL_GROUP*/int sgs[], string name, string descr, color bgColor) {
   ...
   // überpr�fen, ob die angegebene Gruppe bereits existiert und dabei den ersten freien Index ermitteln
