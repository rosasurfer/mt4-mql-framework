...
double iMama(double price, double fastLimit, double slowLimit, double &retFama, int r, int bars) {
   ...
   workMama[r][_smooth]    = (r > 3) ? (4*workMama[r][_price] + 3*workMama[r-1][_price] + 2*workMama[r-2][_price] + workMama[r-3][_price])/10 : price;
   ...
   if (workMama[r][_Re]!=0 && workMama[r][_Im]!=0) {
      workMama[r][_period] = 360 / (MathArctan(workMama[r][_Im]/workMama[r][_Re]) * rad2degree);
   }
   ...
   if (workMama[r][_I1]!=0) {
      workMama[r][_phase] = MathArctan(workMama[r][_Q1]/workMama[r][_I1]) * rad2degree;
   }
   ...
   workMama[r][_mama] = (r==0) ? price :     workMama[r][_sa] * workMama[r][_price] + (1 -     workMama[r][_sa]) * workMama[r-1][_mama];
   workMama[r][_fama] = (r==0) ? price : 0.5*workMama[r][_sa] * workMama[r][_mama]  + (1 - 0.5*workMama[r][_sa]) * workMama[r-1][_fama];

   retFama = workMama[r][_fama];
...
double getPrice(ENUM_APPLIED_PRICE tprice, const double &open[], const double &close[], const double &high[], const double &low[], int i, int _bars) {
   if (i >= 0) {
      switch (tprice) {
         ...
         case PRICE_MEDIAN:   return (high[i]+low[i])/2;
         case PRICE_TYPICAL:  return (high[i]+low[i]+close[i])/3;
         case PRICE_WEIGHTED: return (high[i]+low[i]+close[i]+close[i])/4;
      }
   }
   return 0;
}
