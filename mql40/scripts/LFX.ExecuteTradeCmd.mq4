...
bool OpenLfxOrder.Execute(/*LFX_ORDER*/int lo[], int &subPositions) {
   ...
   for (int retry, i=0; i < symbolsSize; i++) {
      ...
      // Schrittweite mit zunehmender Lotsize über MODE_LOTSTEP hinaus erh�hen (entspricht Algorithmus in ChartInfos-Indikator)
...
bool OpenLfxOrder.Save(/*LFX_ORDER*/int lo[], bool isOpenError) {
   ...
   if (!LFX.SaveOrder(lo, NULL, F_ERR_CONCURRENT_MODIFICATION)) {    // ERR_CONCURRENT_MODIFICATION abfangen
      ...
      // (2.2) gespeicherten OpenError immer überschreiben (auch bei fehlgeschlagener Ausf�hrung), um ein evt. "Mehr" an Ausf�hrungsdetails nicht zu verlieren
...
/**
 * Schickt eine Benachrichtigung über Erfolg/Mi�erfolg der Orderausf�hrung an die interessierten Listener.
 *
 * @param  _In_ LFX_ORDER lo[] - LFX-Order
 *
 * @return bool - success status
...
bool CloseLfxOrder.Save(/*LFX_ORDER*/int lo[], bool isCloseError) {
   ...
   if (!LFX.SaveOrder(lo, NULL, F_ERR_CONCURRENT_MODIFICATION)) {    // ERR_CONCURRENT_MODIFICATION abfangen
      ...
      // (2.2) gespeicherten CloseError immer überschreiben (auch bei fehlgeschlagener Ausf�hrung), um ein evt. "Mehr" an Ausf�hrungsdetails nicht zu verlieren
...
/**
 * Schickt eine Benachrichtigung über Erfolg/Mi�erfolg der Orderausf�hrung ans LFX-Terminal.
 *
 * @param  _In_ LFX_ORDER lo[] - LFX-Order
 *
 * @return bool - success status
