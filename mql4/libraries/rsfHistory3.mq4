...
/**
 * Gibt ein Handle für das gesamte HistorySet eines Symbols zurück. Wurde das HistorySet vorher nicht mit HistorySet3.Create() erzeugt, muß
 ...
 * @param  string server - Name des Serververzeichnisses, in dem das Set gespeichert wird (default: aktuelles Serververzeichnis)
 *
 * @return int - • Set-Handle oder -1, falls kein HistoryFile dieses Symbols existiert. In diesem Fall muß mit HistorySet3.Create() ein neues
 *                 Set erzeugt werden.
...
int HistorySet3.Get(string symbol, string server = "") {
   ...
   for (i=0; i < sizeOfPeriods; i++) {
      ...
      if (IsFileA(fullFileName)) {                                      // wenn Datei existiert, öffnen
         ...
         if (fileSize < HISTORY_HEADER.size) {
            FileClose(hFile);
            logWarn("HistorySet3.Get(5)  invalid history file \""+ mqlFileName +"\" found (size="+ fileSize +")");
            continue;
         }
   ...
   if (!catch("HistorySet3.Get(6)  [hstSet="+ DoubleQuoteStr(symbol) +"]"))
      return(-1);
   return(NULL);
}
...
bool HistorySet3.Close(int hSet) {
   ...
   for (int i=0; i < sizeOfPeriods; i++) {
      if (hs.hFile[hSet][i] > 0) {                                   // alle offenen Dateihandles schließen
         if (!HistoryFile3.Close(hs.hFile[hSet][i])) return(false);
         hs.hFile[hSet][i] = -1;
      }
   }
   hs.hSet[hSet] = -1;
...
int HistoryFile3.Open(string symbol, int timeframe, string copyright, int digits, int format, int mode, string server="") {
   ...
   if (!read_only) /*&&*/ if (!IsDirectoryA(fullHstDir)) return(_NULL(catch("HistoryFile3.Open(6)  directory "+ DoubleQuoteStr(fullHstDir) +" doesn't exist ("+ symbol +","+ PeriodDescription(timeframe) +")", ERR_RUNTIME_ERROR)));
 ...
 * Die Datei muß vorher mit HistoryFile3.Open() geöffnet worden sein.
 *
...
int HistoryFile3.FindBar(int hFile, datetime time, bool &lpBarExists[]) {
   ...
   return(_EMPTY(catch("HistoryFile3.FindBar(6)  bars="+ hf.full.bars[hFile] +", from='"+ TimeToStr(hf.full.from.openTime[hFile], TIME_FULL) +"', to='"+ TimeToStr(hf.full.to.openTime[hFile], TIME_FULL) +"')  time look-up in a timeseries not yet implemented ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_NOT_IMPLEMENTED)));
}
...
bool HistoryFile3.ReadBar(int hFile, int offset, double &bar[]) {
   ...
   if (offset >= hf.full.bars[hFile])   return(!catch("HistoryFile3.ReadBar(6)  invalid parameter offset: "+ offset +" ("+ hf.full.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
...
bool HistoryFile3.WriteBar(int hFile, int offset, double bar[], int flags=NULL) {
   ...
   if (offset > hf.full.bars[hFile])    return(!catch("HistoryFile3.WriteBar(6)  invalid parameter offset: "+ offset +" ("+ hf.full.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   if (ArraySize(bar) != 6)             return(!catch("HistoryFile3.WriteBar(7)  invalid size of parameter bar[]: "+ ArraySize(bar) +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INCOMPATIBLE_ARRAYS));
   ...
   if (offset==hf.full.to.offset      [hFile]) /*&&*/ if (openTime!=hf.full.to.openTime      [hFile]) return(!catch("HistoryFile3.WriteBar(12)  bar["+ offset +"].time="+ TimeToStr(openTime, TIME_FULL) +" collides with hf.full.to.time="                                            + TimeToStr(hf.full.to.openTime      [hFile], TIME_FULL) +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_ILLEGAL_STATE));
   ...
   //                            mit HistoryFile3.InsertBar() eingefügt worden sein, das die entsprechende Lücke zwischen beiden Bars einrichtet.
...
bool HistoryFile3.UpdateBar(int hFile, int offset, double value) {
   ...
   if (offset >= hf.full.bars[hFile])   return(!catch("HistoryFile3.UpdateBar(6)  invalid parameter offset: "+ offset +" ("+ hf.full.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
...
bool HistoryFile3.InsertBar(int hFile, int offset, double bar[], int flags=NULL) {
   ...
   if (offset > hf.full.bars[hFile])    return(!catch("HistoryFile3.InsertBar(6)  invalid parameter offset: "+ offset +" ("+ hf.full.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
   if (ArraySize(bar) != 6)             return(!catch("HistoryFile3.InsertBar(7)  invalid size of parameter data[]: "+ ArraySize(bar) +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INCOMPATIBLE_ARRAYS));
...
bool HistoryFile3.WriteBufferedBar(int hFile, int flags=NULL) {
   ...
   if (offset != hf.full.bars[hFile]-1) return(!catch("HistoryFile3.WriteBufferedBar(6)  invalid hf.bufferedBar.offset: "+ offset +" ("+ hf.full.bars[hFile] +" full bars, symbol="+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_RUNTIME_ERROR));
...
bool HistoryFile3.AddTick(int hFile, datetime time, double value, int flags=NULL) {
   ...
   if (time < hf.full.to.openTime[hFile]) return(!catch("HistoryFile3.AddTick(6)  cannot add tick to a closed bar: tickTime="+ TimeToStr(time, TIME_FULL) +", last bar.time="+ TimeToStr(hf.full.to.openTime[hFile], TIME_FULL) +" ("+ hf.symbol[hFile] +","+ PeriodDescription(hf.period[hFile]) +")", ERR_INVALID_PARAMETER));
