...
int UpdateInstrumentInfos() {
   ...
   ObjectSetText(labels[I_SPREAD_DATA            ],                      DoubleToStr(spreadP/pUnit, pDigits) + _spUnit + ifString(!adr, "", " = "+ NumberToStr(NormalizeDouble(MathDiv(spreadP, adr) * 100, 2), ".1+") +"%/ADR"),  fontSize, fontName, fontColor);
   ObjectSetText(labels[I_COMMISSION             ], "Commission:",                                                                                                                                                                 fontSize, fontName, fontColor);
   ObjectSetText(labels[I_COMMISSION_DATA        ],                      ifString(!commissionM, "-", NumberToStr(commissionPunit, pUnitFormat) + _spUnit +" = "+ DoubleToStr(commissionM, 2) +" "+ accountCurrency +"/lot"),       fontSize, fontName, fontColor);
   ObjectSetText(labels[I_TOTAL_COST             ], "Total cost:",                                                                                                                                                                 fontSize, fontName, fontColor);
   ObjectSetText(labels[I_TOTAL_COST_DATA        ],                      ifString(!totalCostP,  "-", NumberToStr(totalCostPunit, pUnitFormat) + _spUnit) +" = "+ NumberToStr(NormalizeDouble(totalCostPct, 3), ".1+") +"%/TrxVol", fontSize, fontName, fontColor);
...
string InputsToStr() {
   return(StringConcatenate("AccountSize.NumberOfUnits=",     AccountSize.NumberOfUnits,     ";", NL,
                            "AccountSize.MaxUnitRisk.Pct=",   AccountSize.MaxUnitRisk.Pct,   ";", NL,
                            "AccountSize.MaxUsedMargin.Pct=", AccountSize.MaxUsedMargin.Pct, ";")
   );
}
